...
export default function LiveViewer({ searchParams }) {
  ...
  const sendGift = async (g) => {
    ...
    await supabase.from('live_messages').insert({
      ...
      message: `${g.emoji} ${g.name} পাঠালো`, type: 'gift',
      ...
      {showGifts && (
        <div style={{ position:'absolute', bottom:'80px', left:0, right:0, zIndex:30, background:'rgba(13,18,32,.95)', backdropFilter:'blur(12px)', borderTop:'1px solid rgba(255,255,255,.1)', padding:'16px', animation:'slideUp .25s ease' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'8px' }}>
            {GIFTS.map((g, i) => (
              <div key={i} onClick={() => sendGift(g)} style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:'4px', padding:'10px 6px', background:'rgba(255,255,255,.06)', borderRadius:'12px', cursor:'pointer', border:'1px solid rgba(255,255,255,.08)' }}>
