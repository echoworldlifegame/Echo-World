  ...
  const lookupTransferUser = async (addr) => {
    ...
    if (accs) {
      const found = accs.find(a => {
        const h = btoa(a.user_id).replace(/[^a-zA-Z0-9]/g, '').slice(0, 34).toUpperCase()
        return ('EW' + h) === addr.toUpperCase()
      })
            ...
            {walletTab === 'transfer' && (
              <div>
                <div style={{ background:'rgba(0,229,255,0.04)', border:'1px solid rgba(0,229,255,0.15)', borderRadius:'14px', padding:'14px', marginBottom:'12px', fontSize:'11px', color:'#4a5568', lineHeight:'1.8' }}>
                  ...
                  🔒 Receiver এর Wallet Address দিন
                </div>
                <div style={{ background:'#111620', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'16px', padding:'16px', marginBottom:'12px' }}>
                  <div style={{ marginBottom:'12px' }}>
                    <div style={{ fontSize:'10px', color:'#4a5568', fontWeight:'700', marginBottom:'5px' }}>RECEIVER WALLET ADDRESS</div>
                    <input value={transferTo} onChange={e => lookupTransferUser(e.target.value)} placeholder='EW...'
