...
function Certificate({ inv, username, referralLink, onClose }) {
  ...
  const handleShare = async () => {
    ...
    try {
      // Certificate কে image এ convert করে share করো
      ...
      canvas.toBlob(async (blob) => {
        if (blob && navigator.share && navigator.canShare?.({ files: [new File([blob], 'certificate.png', { type: 'image/png' })] })) {
          ...
          // Fallback: image download করো
          const url = URL.createObjectURL(blob)
          const a = document.createElement('a')
          a.href = url
          a.download = 'echo-certificate.png'
          a.click()
          URL.revokeObjectURL(url)
          setTimeout(() => {
            navigator.clipboard?.writeText(shareText)
            alert('✅ Certificate downloaded! Now share the image with the copied text.')
          }, 500)
        }
      }, 'image/png')
    } catch (err) {
      // Fallback: শুধু text copy
      navigator.clipboard?.writeText(shareText)
  ...
  const handleDownload = async () => {
    try {
      ...
      const a = document.createElement('a')
      a.href = url
      a.download = `echo-certificate-${username}.png`
      a.click()
    } catch (err) {
