...
export default function Invest() {
  ...
  const LockWrap = ({ children }) => (
    <div style={{ minHeight:'100vh', background:'linear-gradient(160deg,#020509,#070d1a,#020509)', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'20px', position:'relative', overflow:'hidden' }}>
      ...
      {/* Static trust badges — PIN screen এ animation নেই, keyboard safe */}
      <div style={{ width:'100%', maxWidth:'340px', display:'flex', flexWrap:'wrap', gap:'6px', justifyContent:'center', marginBottom:'20px' }}>
        {['🔒 100% Secure', '💎 Principal returned', '📅 14th & 28th withdrawal', '👥 2-Level referral'].map(t => (
          <div key={t} style={{ background:'rgba(0,229,255,0.04)', border:'1px solid rgba(0,229,255,0.12)', borderRadius:'20px', padding:'5px 12px', fontSize:'10px', color:'#8892a4' }}>{t}</div>
        ))}
      </div>
