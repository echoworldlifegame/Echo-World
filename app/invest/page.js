...
export default function Invest() {
  ...
  // ── 2FA OTP ─────────────────────────────────────────────────
  const sendOtp = async () => {
    setSendingOtp(true); setTwoFAError('')
    const otp = String(Math.floor(100000 + Math.random() * 900000))
    setTwoFAOtp(otp)
    // Support message হিসেবে OTP পাঠাও
    await supabase.from('support_messages').insert({
      user_id: user.id, sender_role: 'admin', sender_name: 'Echo Security',
      message: `🔐 Your Echo World 2FA code: ${otp} — Valid for 5 minutes. Do not share with anyone.`,
      read: false,
    })
    setSendingOtp(false)
    setTwoFAStep('verify')
  ...
  const verify2FA = () => {
    if (twoFAInput.trim() === twoFAOtp) {
      setTwoFAEnabled(true)
      setTwoFAStep(null); setTwoFAInput(''); setTwoFAOtp('')
      alert('✅ 2FA Enabled! Your account is now secured.')
    } else {
      setTwoFAError('❌ Wrong code! Try again.')
    }
  }
  ...
  const submitTransfer = async () => {
    ...
    await supabase.from('investment_accounts').update({
      wallet_balance: (account.wallet_balance || 0) - amt
    }).eq('user_id', user.id)
    // Credit receiver
    const { data: recvAcc } = await supabase.from('investment_accounts').select('wallet_balance, total_earned').eq('user_id', transferUser.user_id).single()
    await supabase.from('investment_accounts').update({
      wallet_balance: (recvAcc?.wallet_balance || 0) + amt,
      total_earned: (recvAcc?.total_earned || 0) + amt,
    }).eq('user_id', transferUser.user_id)
    // Log both sides
    const today = new Date().toISOString().split('T')[0]
    await supabase.from('daily_earnings').insert([
      { user_id: user.id, investment_id: null, amount: -amt, type: 'transfer_out', date: today },
      { user_id: transferUser.user_id, investment_id: null, amount: amt, type: 'transfer_in', date: today },
    ])
    // Notify receiver
    await supabase.from('notifications').insert({
      user_id: transferUser.user_id, from_user_id: null, type: 'system',
      message: `🌐 Echo World: @${username} আপনাকে $${amt} transfer করেছে! Wallet এ যোগ হয়েছে।`,
      read: false,
    })
    alert(`✅ $${amt} transferred to @${transferUser.profiles?.username}!`)
  ...
  const submitReinvest = async () => {
    ...
    await supabase.from('investment_accounts').update({
      wallet_balance: (account.wallet_balance || 0) - amt,
      total_invested: (account.total_invested || 0) + amt,
    }).eq('user_id', user.id)
    ...
    alert(`✅ $${amt} reinvested as ${plan.label} plan!`)
    setReinvestAmt('')
    await loadAll(user.id)
  ...
  return (
    <div style={{ minHeight:'100vh', background: darkMode ? '#050810' : '#f0f4f8', color: darkMode ? '#eef2f7' : '#1a202c', paddingBottom:'90px', transition:'background 0.3s, color 0.3s' }}>
      ...
      {/* ── 2FA Modal ── */}
      {twoFAStep && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.88)', zIndex:3000, display:'flex', alignItems:'center', justifyContent:'center', padding:'24px' }}>
          <div style={{ background:'#111620', borderRadius:'20px', padding:'24px', width:'100%', maxWidth:'300px' }}>
            {twoFAStep === 'info' && (
              <>
                <div style={{ fontSize:'28px', textAlign:'center', marginBottom:'8px' }}>{twoFAEnabled ? '🛡️' : '🔒'}</div>
                <div style={{ fontSize:'15px', fontWeight:'900', color:'#eef2f7', textAlign:'center', marginBottom:'6px' }}>
                  {twoFAEnabled ? '2FA Enabled' : '2FA Security'}
                </div>
                <div style={{ fontSize:'11px', color:'#4a5568', textAlign:'center', lineHeight:'1.7', marginBottom:'20px' }}>
                  {twoFAEnabled
                    ? 'Your account is protected with 2FA. OTP যাবে Support Messages এ।'
                    : 'Extra security layer add করুন। Login এর সময় OTP verify করতে হবে। OTP যাবে Support Messages এ।'}
                </div>
                {twoFAEnabled ? (
                  ...
                  <button onClick={sendOtp} disabled={sendingOtp} style={{ width:'100%', padding:'13px', background:'linear-gradient(135deg,#00e5ff,#00ff88)', border:'none', borderRadius:'14px', color:'#050810', fontSize:'13px', fontWeight:'900', cursor:'pointer', marginBottom:'8px' }}>
                    {sendingOtp ? '⏳ Sending OTP...' : '📨 Enable — Send OTP'}
                  </button>
                )}
            ...
            {twoFAStep === 'verify' && (
              <>
                ...
                <div style={{ fontSize:'15px', fontWeight:'900', color:'#eef2f7', textAlign:'center', marginBottom:'4px' }}>Enter OTP</div>
                <div style={{ fontSize:'11px', color:'#4a5568', textAlign:'center', lineHeight:'1.7', marginBottom:'16px' }}>
                  Support Messages এ ৬ সংখ্যার OTP গেছে। সেখান থেকে দেখে এখানে দিন।
                </div>
                <input
                ...
                <button onClick={() => { setTwoFAStep(null); setTwoFAInput('') }} style={{ width:'100%', padding:'12px', background:'transparent', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'14px', color:'#4a5568', fontSize:'13px', cursor:'pointer' }}>Cancel</button>
              </>
            )}
          </div>
        </div>
