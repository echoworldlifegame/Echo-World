...
export default function Post() {
  ...
  const [capsuleMedia, setCapsuleMedia] = useState('photo')
  ...
  const getAcceptType = () => {
    ...
    if (postType === 'capsule') return capsuleMedia === 'video' ? 'video/*' : 'image/*'
    return 'image/*'
  }

  const showUpload = postType === 'photo' || postType === 'video' || postType === 'capsule'
      ...
      <div style={{padding:'76px 16px 20px',maxWidth:'600px',margin:'0 auto'}}>
        ...
        {postType === 'capsule' && (
          <div style={{marginBottom:'16px'}}>
            <div style={{background:'rgba(255,202,40,0.06)',border:'1px solid rgba(255,202,40,0.2)',borderRadius:'12px',padding:'12px 16px',marginBottom:'12px'}}>
            ...
            <div style={{display:'flex',gap:'8px'}}>
              <button onClick={()=>{setCapsuleMedia('photo');setMediaUrl('');setMediaPreview('')}} style={{
                flex:1,padding:'10px',borderRadius:'10px',border:'none',cursor:'pointer',fontSize:'13px',fontWeight:'600',
                background:capsuleMedia==='photo' ? 'rgba(0,229,255,0.15)' : '#111620',
                color:capsuleMedia==='photo' ? '#00e5ff' : '#4a5568',
                border:capsuleMedia==='photo' ? '1px solid rgba(0,229,255,0.3)' : '1px solid rgba(255,255,255,0.07)',
              }}>📷 Photo</button>
              <button onClick={()=>{setCapsuleMedia('video');setMediaUrl('');setMediaPreview('')}} style={{
                flex:1,padding:'10px',borderRadius:'10px',border:'none',cursor:'pointer',fontSize:'13px',fontWeight:'600',
                background:capsuleMedia==='video' ? 'rgba(255,69,96,0.15)' : '#111620',
                color:capsuleMedia==='video' ? '#ff4560' : '#4a5568',
                border:capsuleMedia==='video' ? '1px solid rgba(255,69,96,0.3)' : '1px solid rgba(255,255,255,0.07)',
              }}>🎥 Video</button>
              <button onClick={()=>{setCapsuleMedia('none');setMediaUrl('');setMediaPreview('')}} style={{
                flex:1,padding:'10px',borderRadius:'10px',border:'none',cursor:'pointer',fontSize:'13px',fontWeight:'600',
                background:capsuleMedia==='none' ? 'rgba(0,255,136,0.1)' : '#111620',
                color:capsuleMedia==='none' ? '#00ff88' : '#4a5568',
                border:capsuleMedia==='none' ? '1px solid rgba(0,255,136,0.25)' : '1px solid rgba(255,255,255,0.07)',
              }}>✍ Text only</button>
            </div>
          </div>
        )}
        ...
        {/* Media Upload */}
        {showUpload && (postType !== 'capsule' || capsuleMedia !== 'none') && (
          <div style={{marginBottom:'16px'}}>
            <input ref={fileRef} type="file" accept={getAcceptType()} onChange={handleFileUpload} style={{display:'none'}}/>
            {!mediaPreview ? (
              <div onClick={()=>fileRef.current.click()} style={{border:'2px dashed rgba(0,229,255,0.3)',borderRadius:'14px',padding:'36px',textAlign:'center',cursor:'pointer',background:'rgba(0,229,255,0.03)'}}>
                <div style={{fontSize:'40px',marginBottom:'8px'}}>
                  {postType==='video' || capsuleMedia==='video' ? '🎥' : '📷'}
                </div>
                <div style={{fontSize:'14px',color:'#00e5ff',fontWeight:'600'}}>
                  Tap to select {postType==='video' || capsuleMedia==='video' ? 'video' : 'photo'}
                </div>
                <div style={{fontSize:'12px',color:'#4a5568',marginTop:'4px'}}>
                  {postType==='video' || capsuleMedia==='video' ? 'MP4, MOV supported' : 'JPG, PNG, GIF supported'}
                </div>
              </div>
            ) : (
              <div style={{position:'relative',borderRadius:'14px',overflow:'hidden'}}>
                ...
                <button onClick={()=>{setMediaPreview('');setMediaUrl('');setMessage('')}} style={{position:'absolute',top:'10px',right:'10px',background:'rgba(0,0,0,0.7)',border:'none',borderRadius:'50%',width:'32px',height:'32px',color:'#fff',cursor:'pointer',fontSize:'16px'}}>✕</button>
                {uploading && (
                  <div style={{position:'absolute',inset:0,background:'rgba(0,0,0,0.65)',display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column',gap:'8px'}}>
        ...
        <textarea value={content} onChange={e=>setContent(e.target.value)}
          placeholder={postType==='capsule' ? '📦 Write a secret message for whoever visits...' : postType==='checkin' ? '📍 What are you doing here?' : '✍ Share something about this place...'}
