...
export default function Profile() {
  ...
  const saveProfile = async () => {
    ...
    await supabase.from('profiles').update({ full_name: editName, bio: editBio, username: editUsername }).eq('id', user.id)
  ...
  const uploadAvatar = async (file) => {
    ...
    const fd = new FormData(); fd.append('file', file); fd.append('upload_preset', UPLOAD_PRESET); fd.append('folder', 'avatars')
    try {
      ...
      {mainView === 'profile' && (
        <div style={{ animation:'fadeUp .3s ease' }}>
          ...
          <div style={{ position:'relative', height:180 }}>
            {/* Cover gradient */}
            <div style={{ position:'absolute', inset:0, background:`linear-gradient(135deg, ${level.color}22, #0d1220 60%)`, overflow:'hidden' }}>
          ...
          <div style={{ background:'#111826', borderRadius:16, padding:'14px 16px', marginBottom:14, border:'1px solid rgba(255,255,255,.06)' }}>
            ...
            {[
              ...
              <div key={key} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'10px 0', borderBottom:'1px solid rgba(255,255,255,.04)' }}>
                ...
                <div onClick={()=>setSettings(p=>({...p,[key]:!p[key]}))}
