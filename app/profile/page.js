...
export default function Profile() {
  ...
  const loadAll = async (uid) => {
    ...
    setInvestments(invs||[])
    // investment_accounts থেকে total_invested নেওয়া বেশি accurate
    const tot = acc?.total_invested || (invs||[]).reduce((s,i)=>s+(parseFloat(i.amount_usd)||0),0)
    ...
    setEchoBalance(echoAcc?.balance || 0)
  ...
  const totalBalance = (investAccount?.wallet_balance||0) + totalEarned + (coinBalance/COIN_RATE)
        ...
        <div style={{ padding:'16px 14px', animation:'fadeUp .3s ease' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:16 }}>
            {[
              ...
              { ic:'🪙', label:'ECHO Balance', val:`${echoBalance.toLocaleString()} ECHO`, color:'#a855f7', sub:`≈ $${(echoBalance*0.001).toFixed(4)}`, onClick:()=>window.location.href='/echo-wallet' },
            ].map((card,i)=>(
            ...
            {investAccount ? (
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
                {[
                  ...
                  ['Balance',    fmtUSD(investAccount.wallet_balance||0)],
          ...
          {investments.length>0&&(
            <div style={{ marginBottom:14 }}>
              ...
              {investments.map((inv,i)=>{
                const rate = parseFloat(inv.daily_rate)||( parseFloat(inv.amount_usd)>=1000?3:parseFloat(inv.amount_usd)>=500?2.5:2)
                const daily = parseFloat(inv.amount_usd)*rate/100
                ...
                return (
                  <div key={inv.id||i} style={{ background:'#111826', borderRadius:14, padding:'14px 16px', marginBottom:8, border:'1px solid rgba(0,229,255,.1)' }}>
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:8 }}>
                      <div>
                        <div style={{ fontSize:13, fontWeight:800, color:'#eef2f7' }}>{parseFloat(inv.amount_usd)>=1000?'💎 Elite':parseFloat(inv.amount_usd)>=500?'🚀 Growth':'🌱 Starter'} Plan</div>
                        <div style={{ fontSize:11, color:'#4a5568', marginTop:2 }}>Invested: {fmtUSD(inv.amount_usd)}</div>
                      </div>
