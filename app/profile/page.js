...
export default function Profile() {
  ...
  const uploadImage = async (file, type) => {
    ...
    if (data.secure_url) {
      ...
      setProfile(prev => ({ ...prev, [field]: data.secure_url }))
    }
    setUploading('')
  }
    ...
    <div style={{minHeight:'100vh',background:'#070a10',color:'#eef2f7'}}>
      ...
      <div style={{paddingTop:'56px',paddingBottom:'90px',maxWidth:'600px',margin:'0 auto'}}>
        ...
        <div style={{position:'relative',height:'180px',background:'linear-gradient(135deg,#0a1828,#0d2040)',overflow:'hidden'}}>
          {profile?.cover_url
            ? <img src={profile.cover_url} style={{width:'100%',height:'100%',objectFit:'cover'}}/>
            : <div style={{width:'100%',height:'100%',background:'linear-gradient(135deg,#070a10,#0a1428,#070a10)',display:'flex',alignItems:'center',justifyContent:'center',color:'#1a2535',fontSize:'60px'}}>🌍</div>
          }
          <div style={{position:'absolute',inset:0,background:'linear-gradient(to bottom,transparent 40%,rgba(7,10,16,0.9))'}}></div>
          ...
          <button
            ...
            style={{position:'absolute',bottom:'12px',right:'12px',background:'rgba(0,0,0,0.7)',border:'1px solid rgba(255,255,255,0.2)',borderRadius:'8px',padding:'7px 14px',color:'#fff',fontSize:'12px',cursor:'pointer',display:'flex',alignItems:'center',gap:'6px'}}>
            {uploading==='cover' ? '⏳ Uploading...' : '📷 Change Cover'}
          </button>
        </div>

        {/* Profile Info */}
        <div style={{padding:'0 16px',marginTop:'-40px',marginBottom:'16px'}}>
          <div style={{display:'flex',alignItems:'flex-end',justifyContent:'space-between',marginBottom:'14px'}}>
            ...
            <div style={{position:'relative'}}>
              ...
              <button
                ...
                style={{position:'absolute',bottom:'0',right:'0',background:'#111620',border:'2px solid #070a10',borderRadius:'50%',width:'26px',height:'26px',fontSize:'12px',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center'}}>
                {uploading==='avatar' ? '⏳' : '📷'}
              </button>
            </div>
            ...
            <div style={{display:'flex',gap:'8px'}}>
              <button onClick={()=>window.location.href='/post'} style={{background:'linear-gradient(135deg,#00e5ff,#00ff88)',border:'none',borderRadius:'8px',padding:'9px 16px',fontSize:'13px',fontWeight:'700',color:'#070a10',cursor:'pointer'}}>
                + Post
              </button>
              <button onClick={()=>window.location.href='/search'} style={{background:'#111620',border:'1px solid rgba(255,255,255,0.1)',borderRadius:'8px',padding:'9px 12px',fontSize:'16px',cursor:'pointer'}}>
                🔍
              </button>
              <button onClick={async()=>{await supabase.auth.signOut();window.location.href='/'}} style={{background:'rgba(255,69,96,0.1)',border:'1px solid rgba(255,69,96,0.3)',borderRadius:'8px',padding:'9px 12px',color:'#ff4560',fontSize:'12px',cursor:'pointer',fontWeight:'600'}}>
                Logout
              </button>
            </div>
          </div>
          ...
          {editMode ? (
            <div>
              <input value={editName} onChange={e=>setEditName(e.target.value)}
                style={{width:'100%',fontSize:'18px',fontWeight:'800',background:'#111620',border:'1px solid rgba(0,229,255,0.3)',borderRadius:'8px',padding:'8px 12px',color:'#eef2f7',outline:'none',boxSizing:'border-box',marginBottom:'8px'}}/>
              <textarea value={editBio} onChange={e=>setEditBio(e.target.value)}
                placeholder="Write your bio..."
                style={{width:'100%',background:'#111620',border:'1px solid rgba(0,229,255,0.3)',borderRadius:'8px',padding:'8px 12px',color:'#eef2f7',fontSize:'13px',outline:'none',resize:'none',fontFamily:'inherit',boxSizing:'border-box',minHeight:'80px',marginBottom:'10px'}}/>
              <button onClick={saveProfile} style={{width:'100%',background:'linear-gradient(135deg,#00e5ff,#00ff88)',border:'none',borderRadius:'10px',padding:'12px',fontSize:'14px',fontWeight:'700',color:'#070a10',cursor:'pointer'}}>
                💾 Save Profile
              </button>
            </div>
          ) : (
            <div>
              <div style={{fontSize:'22px',fontWeight:'800',marginBottom:'2px'}}>{profile?.full_name || profile?.username}</div>
              <div style={{fontSize:'13px',color:'#4a5568',marginBottom:'6px'}}>@{profile?.username}</div>
              {profile?.bio && <div style={{fontSize:'14px',color:'#8892a4',lineHeight:'1.5',marginBottom:'10px'}}>{profile.bio}</div>}
            </div>
          )}

          {/* Stats */}
          <div style={{display:'flex',background:'#111620',borderRadius:'12px',border:'1px solid rgba(255,255,255,0.07)',overflow:'hidden',marginTop:'12px',marginBottom:'16px'}}>
            {[
              ...
              <div key={s.label} style={{flex:1,textAlign:'center',padding:'12px 4px',borderRight:i<3?'1px solid rgba(255,255,255,0.05)':'none'}}>
                <div style={{fontSize:'17px',fontWeight:'800',color:'#00e5ff'}}>{s.value}</div>
          ...
          {/* Level bar */}
          <div style={{background:'#111620',border:'1px solid rgba(255,255,255,0.07)',borderRadius:'12px',padding:'12px',marginBottom:'16px'}}>
          ...
          {/* Tabs */}
          <div style={{display:'flex',gap:'4px',background:'#111620',borderRadius:'12px',padding:'4px',border:'1px solid rgba(255,255,255,0.07)',marginBottom:'16px'}}>
            {[
              {key:'all',label:'All'},
              ...
              <button key={tab.key} onClick={()=>setActiveTab(tab.key)} style={{flex:1,padding:'8px 4px',border:'none',borderRadius:'8px',cursor:'pointer',fontSize:'11px',fontWeight:'600',background:activeTab===tab.key?'linear-gradient(135deg,#00e5ff,#00ff88)':'transparent',color:activeTab===tab.key?'#070a10':'#4a5568'}}>
                {tab.label}
              </button>
            ))}
          </div>
          ...
          {filteredPosts.length === 0 ? (
            <div style={{textAlign:'center',padding:'40px',color:'#4a5568'}}>
              ...
              <button onClick={()=>window.location.href='/post'} style={{marginTop:'16px',background:'linear-gradient(135deg,#00e5ff,#00ff88)',border:'none',borderRadius:'10px',padding:'10px 24px',fontSize:'13px',fontWeight:'700',color:'#070a10',cursor:'pointer'}}>Create Post</button>
            </div>
            ...
            <div key={post.id} style={{background:'#111620',border:'1px solid rgba(255,255,255,0.07)',borderRadius:'14px',marginBottom:'12px',overflow:'hidden'}}>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'10px 14px 6px'}}>
                <div>
                  ...
                  <div style={{fontSize:'10px',color:'#4a5568',marginTop:'2px'}}>{timeAgo(post.created_at)}</div>
                </div>
                <div style={{display:'flex',gap:'6px',alignItems:'center'}}>
                  <span style={{fontSize:'10px',background:'rgba(0,229,255,0.08)',color:'#00e5ff',padding:'2px 8px',borderRadius:'6px'}}>
                    {post.media_type==='capsule'?'📦':post.media_type==='photo'?'📷':post.media_type==='video'?'🎥':post.media_type==='checkin'?'📍':'✍'}
                  </span>
              ...
              {post.media_url && post.media_type==='photo' && <img src={post.media_url} style={{width:'100%',maxHeight:'260px',objectFit:'cover',display:'block'}}/>}
              {post.media_url && post.media_type==='video' && <video src={post.media_url} controls style={{width:'100%',maxHeight:'260px',display:'block'}}/>}
              {post.media_type==='capsule' && (
                <div style={{margin:'0 14px 10px',background:'rgba(255,202,40,0.06)',border:'1px solid rgba(255,202,40,0.2)',borderRadius:'10px',padding:'12px',display:'flex',gap:'10px',alignItems:'center'}}>
                  ...
                  <div style={{fontSize:'12px',color:'#ffca28'}}>🔒 Time Capsule — locked</div>
                </div>
              )}
              {post.content && <div style={{padding:'8px 14px 12px',fontSize:'13px',color:'#8892a4',lineHeight:'1.5'}}>{post.content}</div>}
              ...
              {deleteConfirm===post.id && (
                <div style={{margin:'0 14px 12px',background:'rgba(255,69,96,0.08)',border:'1px solid rgba(255,69,96,0.2)',borderRadius:'10px',padding:'12px'}}>
                  ...
                  <div style={{display:'flex',gap:'8px'}}>
                    <button onClick={()=>deletePost(post.id)} style={{flex:1,background:'#ff4560',border:'none',borderRadius:'8px',padding:'8px',color:'#fff',fontSize:'13px',fontWeight:'700',cursor:'pointer'}}>Delete</button>
