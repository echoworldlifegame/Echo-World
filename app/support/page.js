...
export default function SupportChat() {
  ...
  const loadConversations = async () => {
    const { data } = await supabase
      .from('support_messages')
      .select('user_id, message, created_at, read, sender_role, sender_name, profiles!support_messages_user_id_fkey(username, full_name, avatar_url)')
  ...
  if (loading) return (
    <div style={{ height: '100vh', background: '#050810', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#00e5ff', fontSize: '14px' }}>
      Loading...
    </div>
  )
  ...
  if (isAdmin) return (
    <div style={{ height: '100vh', background: '#070a12', color: '#eef2f7', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '14px 16px', background: 'rgba(7,10,18,0.97)', borderBottom: '1px solid rgba(0,229,255,0.12)', display: 'flex', alignItems: 'center', gap: '12px' }}>
        ...
        {/* Admin name badge */}
        {selectedUserId && (
          <div onClick={() => setShowNameEdit(p => !p)} style={{ background: 'rgba(0,229,255,0.08)', border: '1px solid rgba(0,229,255,0.2)', borderRadius: '10px', padding: '5px 10px', cursor: 'pointer', fontSize: '11px', color: '#00e5ff', fontWeight: '700' }}>
            🪪 {adminName}
          </div>
        )}
      ...
      {/* Admin name edit popup */}
      {showNameEdit && selectedUserId && (
        <div style={{ background: '#111826', border: '1px solid rgba(0,229,255,0.2)', margin: '0 12px', borderRadius: '12px', padding: '12px', display: 'flex', gap: '8px', alignItems: 'center' }}>
          <input
            value={adminName}
            onChange={e => setAdminName(e.target.value)}
            placeholder="Your display name..."
            style={{ flex: 1, background: '#0c1018', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '10px', padding: '8px 12px', color: '#eef2f7', fontSize: '13px', outline: 'none' }}
          />
          <button onClick={() => setShowNameEdit(false)}
            style={{ padding: '8px 14px', background: 'linear-gradient(135deg,#00e5ff,#00ff88)', border: 'none', borderRadius: '10px', color: '#050810', fontSize: '12px', fontWeight: '800', cursor: 'pointer' }}>
            Save
          </button>
        </div>
      )}
              ...
              <div key={conv.user_id}
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: '13px', fontWeight: '700', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span>@{uname}</span>
                    {unreadMap[conv.user_id] > 0 && (
                      <span style={{ background: '#00e5ff', color: '#050810', borderRadius: '20px', padding: '1px 8px', fontSize: '10px', fontWeight: '800' }}>{unreadMap[conv.user_id]} new</span>
                    )}
                  </div>
                  <div style={{ fontSize: '11px', color: '#4a5568', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginTop: '2px' }}>
                    {conv.sender_role === 'admin' ? `${conv.sender_name || 'Support'}: ` : ''}{conv.message}
                  </div>
            ...
            {messages.map((msg, i) => {
              ...
              return (
                <div key={msg.id}>
                  ...
                  <div style={{ display: 'flex', justifyContent: isAdminMsg ? 'flex-end' : 'flex-start', alignItems: 'flex-end', gap: '6px' }}>
                    {/* User avatar (left) */}
                    {!isAdminMsg && (
                      <div style={{ width: '28px', height: '28px', borderRadius: '50%', background: 'linear-gradient(135deg,#a855f7,#ec4899)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', fontWeight: '900', color: '#fff', flexShrink: 0 }}>
                        {displayName[0]?.toUpperCase()}
                      </div>
                    )}
                    <div style={{ maxWidth: '75%' }}>
                      {/* Name label */}
                      <div style={{ fontSize: '10px', fontWeight: '700', marginBottom: '3px', color: isAdminMsg ? '#00e5ff' : '#a855f7', textAlign: isAdminMsg ? 'right' : 'left' }}>
                        {displayName}
                      </div>
                      <div style={{
                        background: isAdminMsg ? 'linear-gradient(135deg,#00e5ff,#00ff88)' : '#1e2535',
                        borderRadius: isAdminMsg ? '18px 18px 4px 18px' : '18px 18px 18px 4px',
                        padding: '10px 14px',
                        fontSize: '13px',
                        color: isAdminMsg ? '#050810' : '#eef2f7',
                    ...
                    {/* Admin avatar (right) */}
                    {isAdminMsg && (
                      <div style={{ width: '28px', height: '28px', borderRadius: '50%', background: 'linear-gradient(135deg,#00e5ff,#00ff88)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '10px', fontWeight: '900', color: '#050810', flexShrink: 0 }}>
                        EW
                      </div>
                    )}
                  </div>
  ...
  return (
    <div style={{ height: '100vh', background: '#050810', color: '#eef2f7', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '14px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
        ...
        {messages.map((msg, i) => {
          ...
          return (
            <div key={msg.id}>
              {showDate && (
                <div style={{ textAlign: 'center', fontSize: '10px', color: '#2a3040', margin: '4px 0' }}>{dateStr(msg.created_at)}</div>
              )}
              <div style={{ display: 'flex', justifyContent: isAdminMsg ? 'flex-start' : 'flex-end', alignItems: 'flex-end', gap: '8px' }}>

                {/* Admin avatar left */}
                {isAdminMsg && (
                  <div style={{ width: '30px', height: '30px', borderRadius: '50%', background: 'linear-gradient(135deg,#00e5ff,#00ff88)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '10px', fontWeight: '900', color: '#050810', flexShrink: 0 }}>
                    EW
                  </div>
                )}
                ...
                <div style={{ maxWidth: '72%' }}>
                  {/* Name label */}
                  <div style={{ fontSize: '10px', fontWeight: '700', marginBottom: '3px', color: isAdminMsg ? '#00e5ff' : '#a855f7', textAlign: isAdminMsg ? 'left' : 'right' }}>
                    {displayName}
                  </div>
                  {/* Bubble */}
                  <div style={{
                    background: isAdminMsg ? '#1a2030' : 'linear-gradient(135deg,#7c3aed,#a855f7)',
                    borderRadius: isAdminMsg ? '4px 18px 18px 18px' : '18px 18px 4px 18px',
                    padding: '10px 14px',
                    fontSize: '13px',
                    color: '#eef2f7',
                    lineHeight: '1.5',
      ...
      <div style={{ padding: '12px 14px 32px', background: 'rgba(5,8,16,0.97)', borderTop: '1px solid rgba(255,255,255,0.07)', display: 'flex', gap: '10px', alignItems: 'flex-end' }}>
        ...
        />
        <button onClick={sendMessage} disabled={!input.trim() || sending}
          style={{ width: '44px', height: '44px', borderRadius: '50%', background: input.trim() ? 'linear-gradient(135deg,#7c3aed,#a855f7)' : 'rgba(255,255,255,0.07)', border: 'none', color: '#fff', fontSize: '18px', cursor: input.trim() ? 'pointer' : 'default', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          ➤
        </button>
      </div>
    </div>
