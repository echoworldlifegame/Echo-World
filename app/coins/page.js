...
export default function CoinStore() {
  ...
  const submitPurchase = async () => {
    ...
    try {
      const { data: insertedPurchase, error: purchaseErr } = await supabase.from('coin_purchases').insert({
        user_id: user.id,
        coins,
        usdt_amount: usdt,
      ...
      // Notify admin
      const { data: adminProf } = await supabase.from('profiles')
        .select('id').eq('username', 'admin').single().catch(()=>({data:null}))
      const { data: adminAuth } = await supabase.from('profiles')
        .select('id').ilike('full_name','%admin%').limit(1).single().catch(()=>({data:null}))
      // Send notification to admin email profile
      const adminId = adminProf?.id || adminAuth?.id
      if (adminId) {
        await supabase.from('notifications').insert({
          user_id: adminId, type: 'system', read: false,
          message: `🪙 নতুন Coin Purchase Request! User: ${profile?.username || user.id.slice(0,8)} — ${coins} Coins ($${usdt} USDT). TRX: ${trxHash.slice(0,12) || 'Screenshot'}`
        })
      }
      // Also store in a way admin can see via coin_purchases table (already done above)
      setSubmitted(true)
      setStep('submitted')
    } catch (e) {
