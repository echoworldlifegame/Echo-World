...
export default function CoinStore() {
  ...
  const submitPurchase = async () => {
    ...
    try {
      ...
      // Admin alert: store in admin_alerts table (admin panel reads this)
      try {
        await supabase.from('admin_alerts').insert({
          type: 'coin_purchase',
          user_id: user.id,
          username: profile?.username || user.id.slice(0,8),
          message: `🪙 ${profile?.username || 'User'} — ${coins} Coins ($${usdt} USDT)${trxHash ? ' | TRX: '+trxHash.slice(0,20) : ' | Screenshot'}`,
          ref_id: insertedPurchase?.id || null,
          read: false,
          created_at: new Date().toISOString()
        })
      } catch(_) { /* alert optional — purchase already saved */ }
