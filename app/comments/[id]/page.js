...
export default function Comments({ params }) {
  ...
  const sendComment = async () => {
    ...
    if (comment) {
      ...
      await supabase.rpc('increment_comments', { pid: params.id }).catch(async () => {
  ...
  return (
    <div style={{minHeight:'100vh',background:'#070a10',color:'#eef2f7',display:'flex',flexDirection:'column'}}>
      ...
      {/* COMMENTS LIST */}
      <div style={{flex:1,padding:'12px 16px',paddingBottom:'100px',overflowY:'auto'}}>
        {loading ? (
          ...
          <div key={comment.id} style={{marginBottom:'16px'}}>
            <div style={{display:'flex',gap:'10px'}}>
              ...
              <div style={{flex:1}}>
                ...
                <div style={{display:'flex',gap:'16px',padding:'4px 4px 0',alignItems:'center'}}>
                  ...
                  <button onClick={()=>handleLikeComment(comment.id,comment.likes_count)}
                    style={{background:'none',border:'none',color:likedComments[comment.id]?'#ff4560':'#4a5568',fontSize:'11px',cursor:'pointer',padding:0}}>
                    ❤️ {comment.likes_count||0}
                  </button>
                  <button onClick={()=>{setReplyTo(comment);setTimeout(()=>inputRef.current?.focus(),100)}}
                    style={{background:'none',border:'none',color:'#4a5568',fontSize:'11px',cursor:'pointer',padding:0}}>
                    Reply
                  </button>
                </div>
              </div>
            ...
            {getReplies(comment.id).map(reply => (
              <div key={reply.id} style={{display:'flex',gap:'8px',marginTop:'8px',paddingLeft:'48px'}}>
                <div style={{width:'30px',height:'30px',borderRadius:'50%',overflow:'hidden',background:'linear-gradient(135deg,#00e5ff,#00ff88)',flexShrink:0,display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer'}}
                  onClick={()=>window.location.href=`/user/${reply.profiles?.id}`}>
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{display:'flex',gap:'12px',padding:'3px 4px 0'}}>
                    ...
                    <button onClick={()=>handleLikeComment(reply.id,reply.likes_count)}
                      style={{background:'none',border:'none',color:likedComments[reply.id]?'#ff4560':'#4a5568',fontSize:'10px',cursor:'pointer',padding:0}}>
      ...
      <div style={{position:'fixed',bottom:0,left:0,right:0,background:'rgba(7,10,16,0.98)',borderTop:'1px solid rgba(255,255,255,0.07)',padding:'10px 16px 24px'}}>
        {replyTo && (
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'6px',padding:'4px 10px',background:'rgba(0,229,255,0.06)',borderRadius:'8px'}}>
        ...
        <div style={{display:'flex',gap:'10px',alignItems:'center'}}>
          ...
          <button
            ...
            style={{width:'46px',height:'46px',borderRadius:'50%',background:text.trim()?'linear-gradient(135deg,#00e5ff,#00ff88)':'rgba(255,255,255,0.05)',border:'none',fontSize:'20px',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,transition:'all 0.2s'}}>
            {sending?'⏳':'↗'}
          </button>
        </div>
      </div>
