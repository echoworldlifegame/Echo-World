...
export default function Comments({ params }) {
  ...
  const handleSupport = async () => {
    ...
    if (supported) {
      ...
      setSupported(true)
      await supabase.from('notifications').insert({
        user_id: post.user_id, from_user_id: user.id, type: 'follow'
      })
    }
  }
    ...
    if (c) {
      ...
      if (post.user_id !== user.id) {
        await supabase.from('notifications').insert({
          ...
          post_id: params.id, message: commentText.trim().slice(0, 100)
        })
      }
    }
    setCommentText('')
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#070a10', color: '#eef2f7', paddingBottom: '140px' }}>

      {/* HEADER */}
      <div style={{ position: 'fixed', top: 0, left: 0, right: 0, background: 'rgba(7,10,16,0.97)', borderBottom: '1px solid rgba(255,255,255,0.07)', zIndex: 100, display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', height: '54px' }}>
        <button onClick={() => window.history.back()}
          style={{ background: 'none', border: 'none', color: '#00e5ff', fontSize: '22px', cursor: 'pointer', padding: '0', display: 'flex', alignItems: 'center' }}>
          ←
        </button>
        <div style={{ fontSize: '16px', fontWeight: '800' }}>Post</div>
      </div>
      ...
      <div style={{ paddingTop: '54px' }}>
        ...
        {/* POST */}
        {post && (
          <div style={{ background: '#111620', border: '1px solid rgba(255,255,255,0.07)', margin: '12px', borderRadius: '16px', overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '12px 14px' }}>
              <div onClick={() => window.location.href = `/user/${post.profiles?.id}`}
                style={{ width: '42px', height: '42px', borderRadius: '50%', background: 'linear-gradient(135deg,#00e5ff,#00ff88)', overflow: 'hidden', flexShrink: 0, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {post.profiles?.avatar_url
                  ? <img src={post.profiles.avatar_url} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  : <span style={{ fontWeight: '800', color: '#070a10' }}>{(post.profiles?.full_name || 'E')[0].toUpperCase()}</span>
                }
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: '700', fontSize: '14px' }}>{post.profiles?.full_name || post.profiles?.username}</div>
                <div style={{ fontSize: '11px', color: '#4a5568' }}>@{post.profiles?.username} · {timeAgo(post.created_at)}</div>
              </div>
              {post.user_id !== user?.id && (
                <button onClick={handleSupport}
                  style={{ padding: '6px 14px', borderRadius: '20px', border: `1px solid ${supported ? 'rgba(0,229,255,0.3)' : 'rgba(255,255,255,0.15)'}`, background: supported ? 'rgba(0,229,255,0.08)' : 'transparent', color: supported ? '#00e5ff' : '#8892a4', fontSize: '12px', fontWeight: '600', cursor: 'pointer' }}>
            ...
            {post.media_url && post.media_type === 'video' && (
              <video src={post.media_url} controls playsInline style={{ width: '100%', maxHeight: '450px', display: 'block', background: '#000' }} />
            )}

            {post.content && <div style={{ padding: '10px 14px', fontSize: '14px', color: '#c0c8d8', lineHeight: '1.6' }}>{post.content}</div>}
            {post.hashtags && <div style={{ padding: '0 14px 10px', fontSize: '12px', color: '#00e5ff' }}>{post.hashtags}</div>}

            <div style={{ display: 'flex', padding: '4px 6px 10px', borderTop: '1px solid rgba(255,255,255,0.04)', gap: '4px' }}>
              <button onClick={handleLikePost}
                style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '8px 14px', border: 'none', background: 'none', cursor: 'pointer', color: liked ? '#ff4560' : '#4a5568', fontSize: '13px', fontWeight: '600' }}>
        ...
        <div style={{ padding: '0 12px' }}>
          <div style={{ fontSize: '13px', color: '#4a5568', fontWeight: '700', letterSpacing: '1px', marginBottom: '12px' }}>
          ...
          {comments.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '40px 20px', color: '#4a5568' }}>
              ...
              <div style={{ fontSize: '12px', marginTop: '4px' }}>Be the first to comment!</div>
            </div>
          ) : (
            topComments.map(comment => (
              <div key={comment.id} style={{ marginBottom: '12px' }}>
                <div style={{ display: 'flex', gap: '10px' }}>
                  <div onClick={() => window.location.href = `/user/${comment.profiles?.id}`}
                    style={{ width: '38px', height: '38px', borderRadius: '50%', overflow: 'hidden', background: 'linear-gradient(135deg,#00e5ff,#00ff88)', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '14px', padding: '10px 12px', marginBottom: '4px' }}>
                      <div style={{ fontSize: '12px', fontWeight: '700', color: '#00e5ff', marginBottom: '4px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '12px', alignItems: 'center', paddingLeft: '4px' }}>
                      ...
                      <button onClick={() => { setReplyTo({ id: comment.id, username: comment.profiles?.username }); inputRef.current?.focus() }}
                        style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#4a5568', fontSize: '11px', fontWeight: '600' }}>
                        Reply
                      </button>
                      {comment.user_id === user?.id && (
                        <button onClick={() => deleteComment(comment.id)}
                          style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#ff4560', fontSize: '11px' }}>
                          Delete
                        </button>
                      )}
                    </div>
                    ...
                    {getReplies(comment.id).map(reply => (
                      <div key={reply.id} style={{ display: 'flex', gap: '8px', marginTop: '8px', paddingLeft: '8px', borderLeft: '2px solid rgba(0,229,255,0.15)' }}>
                        <div onClick={() => window.location.href = `/user/${reply.profiles?.id}`}
                          style={{ width: '30px', height: '30px', borderRadius: '50%', overflow: 'hidden', background: 'linear-gradient(135deg,#00e5ff,#00ff88)', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
                          ...
                          <div style={{ display: 'flex', gap: '10px', alignItems: 'center', paddingLeft: '4px' }}>
                            ...
                            {reply.user_id === user?.id && (
                              <button onClick={() => deleteComment(reply.id)}
                                style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#ff4560', fontSize: '10px' }}>
                                Delete
                              </button>
                            )}
                          </div>
      ...
      <div style={{ position: 'fixed', bottom: '68px', left: 0, right: 0, background: 'rgba(7,10,16,0.98)', borderTop: '1px solid rgba(255,255,255,0.07)', padding: '10px 12px', zIndex: 200 }}>
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          <div style={{ width: '34px', height: '34px', borderRadius: '50%', background: 'linear-gradient(135deg,#00e5ff,#00ff88)', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
