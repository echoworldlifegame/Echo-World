...
export default function Comments() {
    ...
    supabase.auth.getSession().then(async ({ data }) => {
      ...
      try {
        await fetch('/api/algorithm/interact', {
          ...
          body: JSON.stringify({ postId: id, type: 'comment_view', source: 'comments' })
        })
      } catch {}
    })
  }, [id])
    ...
    if (liked) {
      ...
      try {
        await fetch('/api/algorithm/interact', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ postId: post.id, type: 'like', source: 'comments' })
        })
      } catch {}
    }
  }
    ...
    if (newComment) {
      ...
      try {
        await fetch('/api/algorithm/interact', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ postId: post.id, type: 'comment', source: 'comments' })
        })
      } catch {}
    }
  ...
  const handleShare = () => {
    fetch('/api/algorithm/interact', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ postId: post.id, type: 'share', source: 'comments' })
    }).catch(() => {})
    if (navigator.share) {
