...
export default function Map() {
  ...
  const loadLeaflet = (u, p) => {
    ...
    script.onload = () => {
      // Load leaflet-rotate plugin after leaflet
      const rotScript = document.createElement('script')
      rotScript.src = 'https://unpkg.com/leaflet-rotate@0.2.8/dist/leaflet-rotate-src.js'
      rotScript.onload = () => setTimeout(() => setupMap(u, p), 100)
      rotScript.onerror = () => setTimeout(() => setupMap(u, p), 100) // fallback if plugin fails
      document.head.appendChild(rotScript)
    }
  ...
  const setupMap = (u, p) => {
    ...
    tileLayer.current = L.tileLayer(mapStyles.street.url, { maxZoom: 19, attribution: mapStyles.street.attr }).addTo(map)
    L.control.zoom({ position: 'bottomright' }).addTo(map)

    map.on('dragstart', () => { isDragging.current = true })

    // Map click → show destination + route
    map.on('click', async (e) => {
      ...
      const html = `<div style="display:flex;flex-direction:column;align-items:center;cursor:pointer">
        <div style="width:38px;height:38px;border-radius:50%;background:linear-gradient(135deg,#ff4560,#ff8c69);border:3px solid #fff;display:flex;align-items:center;justify-content:center;font-size:18px;box-shadow:0 4px 20px rgba(255,69,96,0.6);animation:bounce 0.6s ease alternate infinite">📍</div>
      ...
      clickMarker.current = L.marker([lat, lng], {
        icon: L.divIcon({ html, className: '', iconAnchor: [19, 48] }),
        zIndexOffset: 900,
      }).addTo(map)
      ...
      try {
        const res = await fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}`, { headers: { 'Accept-Language': 'bn,en' } })
        const data = await res.json()
        if (data.display_name) placeName = data.display_name.split(',').slice(0,3).join(', ')
      } catch {}

      setClickDest({ lat, lng, name: placeName })
      setNavDest({ lat, lng, name: placeName })
      setShowNavPanel(true)
      if (userPosRef.current) {
        fetchAllRoutes({ lat, lng, name: placeName })
      }
    })
  ...
  const clearNav = () => {
    ...
    if (clickMarker.current) { map.removeLayer(clickMarker.current); clickMarker.current = null }
    setNavDest(null)
    setNavRoutes(null)
    setNavQuery('')
    setNavResults([])
  ...
  const changeMapStyle = (styleKey) => {
    ...
    tileLayer.current = L.tileLayer(mapStyles[styleKey].url, { maxZoom: 19, attribution: mapStyles[styleKey].attr }).addTo(map)
  }
  ...
  return (
    <div style={{ height: '100vh', background: '#1a2035', position: 'relative', overflow: 'hidden' }}>
      <style>{`
        ...
        @keyframes echoRing { 0%{transform:scale(0.8);opacity:0.8} 100%{transform:scale(2.5);opacity:0} }
      ...
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 500, padding: '12px 12px 0' }}>
        <div style={{ background: 'rgba(10,15,26,0.88)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '18px', padding: '10px 14px', display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            {speed > 0.3
              ? <div style={{ fontSize: '10px', color: '#00e5ff', fontWeight: '700' }}>{transportLabel[transport]} · {(speed * 3.6).toFixed(1)} km/h</div>
              : <div style={{ fontSize: '10px', color: '#4a5568' }}>Map এ চাপুন → রুট দেখুন</div>
            }
          </div>
      ...
      {/* CENTER + COMPASS RESET */}
      <div style={{ position: 'absolute', right: '12px', bottom: '200px', zIndex: 500, display: 'flex', flexDirection: 'column', gap: '8px' }}>
        <button onClick={() => {
          const map = mapInstance.current
          if (map && map.setBearing) map.setBearing(0)
        }} style={{ width: '46px', height: '46px', borderRadius: '50%', background: 'rgba(10,15,26,0.92)', backdropFilter: 'blur(16px)', border: '2px solid rgba(255,165,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: '20px', boxShadow: '0 4px 20px rgba(255,165,0,0.2)' }}>🧭</button>
