      ...
      <div style={{ padding:'115px 14px 40px' }}>
        ...
        {tab === 'coins' && (
          <div style={{ padding:'0 12px' }}>
            ...
            {coinDebug && (
              <div style={{ background:'rgba(255,100,0,.1)', border:'1px solid rgba(255,100,0,.3)', borderRadius:'10px', padding:'12px', marginBottom:'12px', fontSize:'11px', fontFamily:'monospace' }}>
                <div style={{ color:'#ff6b35', fontWeight:'800', marginBottom:'6px' }}>🔍 Debug Info</div>
                <div style={{ color: coinDebug.purchaseError ? '#ff4560' : '#00ff88' }}>
                  Purchases: {coinDebug.purchaseCount} rows | Error: {coinDebug.purchaseError || 'none'}
                </div>
                <div style={{ color: coinDebug.profileError ? '#ff4560' : '#00ff88' }}>
                  Profiles: {coinDebug.profileCount || 0} found | Error: {coinDebug.profileError || 'none'}
                </div>
                {coinDebug.rawFirst && (
                  <div style={{ color:'#8892a4', marginTop:'4px', wordBreak:'break-all' }}>
                    First row: {JSON.stringify(coinDebug.rawFirst).slice(0,200)}
                  </div>
                )}
                <button onClick={loadCoinPurchases} style={{ marginTop:'8px', padding:'4px 10px', background:'rgba(0,229,255,.15)', border:'1px solid rgba(0,229,255,.3)', borderRadius:'6px', color:'#00e5ff', cursor:'pointer', fontSize:'11px' }}>
                  🔄 Reload
                </button>
              </div>
            )}
