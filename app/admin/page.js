            ...
            {refTab === 'commission' && (
              <div>
                <div style={{ background:'rgba(0,255,136,0.05)', border:'1px solid rgba(0,255,136,0.2)', borderRadius:'16px', padding:'18px', marginBottom:'14px' }}>
                  ...
                  <div style={{ background:'rgba(255,165,0,0.06)', border:'1px solid rgba(255,165,0,0.15)', borderRadius:'10px', padding:'10px', fontSize:'11px', color:'#ffa500', lineHeight:'1.7', marginBottom:'12px' }}>
                    ⚠️ এই rates শুধু UI তে save হয়। Daily earnings Edge Function deploy করলে এই rates সেখানে manually set করতে হবে।
                  </div>
                  <button onClick={() => alert(`✅ Saved: L1=${commRates.l1}%, L2=${commRates.l2}%\n\nEdge Function এ এই values set করুন:\nL1_RATE=${commRates.l1}\nL2_RATE=${commRates.l2}`)}
                ...
                <div style={{ background:'#111826', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'14px', padding:'14px' }}>
                  ...
                  {[
                    ...
                    { step:'3', text:'Admin approves User B's deposit → valid_referrals+1 for User A', color:'#ffa500' },
                    { step:'4', text:'Daily: User A earns L1 commission on User B's earnings', color:'#00e5ff' },
                    { step:'5', text:'User A's referrer earns L2 commission on User B's earnings', color:'#00ff88' },
                  ].map(s => (
