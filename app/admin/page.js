...
export default function AdminPage() {
  ...
  const applyAdjust = async () => {
    ...
    try {
      ...
      const { data: freshAcc } = await supabase.from('investment_accounts')
        .select('wallet_balance, total_earned').eq('user_id', selAcc.user_id).single()
      const curBal   = parseFloat(freshAcc?.wallet_balance || 0)
      const curEarned = parseFloat(freshAcc?.total_earned || 0)

      if (adjType === 'deduct' && amt > curBal) {
        alert(`কাটা যাবে না! Available: $${curBal.toFixed(2)}, চাইছ: $${amt.toFixed(2)}`); return
      }

      const newBal    = adjType === 'add'
        ? parseFloat((curBal + amt).toFixed(8))
        : parseFloat((curBal - amt).toFixed(8))
      const newEarned = adjType === 'add'
        ? parseFloat((curEarned + amt).toFixed(8))
        : curEarned

      const { error } = await supabase.from('investment_accounts')
        .update({ wallet_balance: newBal, total_earned: newEarned })
        .eq('user_id', selAcc.user_id)
      ...
      await supabase.from('daily_earnings').insert({
        ...
        note: `Admin: ${adjType==='add'?'+':'-'}$${amt.toFixed(2)} | Previous: $${curBal.toFixed(2)} → New: $${newBal.toFixed(2)}`
      })
  ...
  const loadCoinPurchases = async () => {
    const { data, error } = await supabase
      ...
      .order('created_at', { ascending: false })
    if (error) console.error('coinPurchases error:', error)
  ...
  const approveCoin = async (purchase) => {
    try {
      ...
      const newBal = (prof?.coin_balance || 0) + purchase.coins
      await supabase.from('profiles').update({ coin_balance: newBal }).eq('id', purchase.user_id)
      ...
      await supabase.from('coin_transactions').insert({
        user_id: purchase.user_id, amount: purchase.coins,
        type: 'purchase', ref_id: purchase.id,
        note: `${purchase.coins} coins purchased (${purchase.usdt_amount} USDT)`
      })
      await sendSystemNotif(purchase.user_id,
        `🪙 তোমার ${purchase.coins} Coins approve হয়েছে! Balance: ${newBal} coins`)
  ...
  const grantCoins = async (userId, amount) => {
    ...
    const newBal = (prof?.coin_balance || 0) + parseInt(amount)
    await supabase.from('profiles').update({ coin_balance: newBal }).eq('id', userId)
    await supabase.from('coin_transactions').insert({
      user_id: userId, amount: parseInt(amount),
      type: 'admin_grant', note: `Admin granted ${amount} coins`
    })
    await sendSystemNotif(userId, `🎁 Admin তোমাকে ${amount} Coins দিয়েছে! Balance: ${newBal} coins`)
    alert(`✅ ${amount} Coins দেওয়া হয়েছে!`)
    await loadCoinPurchases()
  }
        ...
        {tab === 'coins' && (
          <div style={{ padding:'0 12px' }}>
            ...
            <div style={{ background:'rgba(255,215,0,.06)', border:'1px solid rgba(255,215,0,.15)', borderRadius:'14px', padding:'14px', marginBottom:'14px' }}>
              ...
              <div style={{ display:'flex', gap:'8px' }}>
                <input id="coinGrantUser" placeholder='Username বা নাম...'
                  style={{ flex:1, background:'rgba(255,255,255,.06)', border:'1px solid rgba(255,255,255,.1)', borderRadius:'10px', padding:'10px 12px', color:'#eef2f7', fontSize:'13px', outline:'none' }}/>
                <input id="coinGrantAmt" type='number' placeholder='Coins' min='1'
                ...
                <button onClick={async()=>{
                  const uInput = document.getElementById('coinGrantUser').value.trim()
                  const amt = parseInt(document.getElementById('coinGrantAmt').value)
                  if (!uInput || !amt) return
                  const {data:p} = await supabase.from('profiles').select('id,username').or(`username.ilike.%${uInput}%,full_name.ilike.%${uInput}%`).limit(1).single()
                  if (!p) { alert('User পাওয়া যায়নি'); return }
                  await grantCoins(p.id, amt)
                  document.getElementById('coinGrantUser').value=''
                  document.getElementById('coinGrantAmt').value=''
