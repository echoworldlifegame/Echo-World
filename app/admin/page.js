...
const ADMIN_EMAIL  = 'mdakibgoodboy4@gmail.com' // primary
const ADMIN_EMAILS = ['mdakibgoodboy4@gmail.com', 'hamja01782@gmail.com']
...
function NIDPanel({ supabase, sendSystemNotif }) {
  const [nidList, setNidList] = React.useState([])
  const [loading, setLoading] = React.useState(true)

  React.useEffect(() => {
    supabase.from('profiles')
      .select('id, username, full_name, avatar_url, nid_front_url, nid_back_url, nid_status, nid_submitted_at')
      .eq('nid_status', 'pending')
      .order('nid_submitted_at', { ascending: false })
      .then(({ data }) => { setNidList(data || []); setLoading(false) })
  }, [])

  const approve = async (u) => {
    await supabase.from('profiles').update({ nid_status: 'approved', is_monetized: true }).eq('id', u.id)
    await sendSystemNotif(u.id, '✅ তোমার NID অনুমোদিত হয়েছে! Monetization চালু হয়েছে।')
    setNidList(p => p.filter(x => x.id !== u.id))
    alert(`✅ @${u.username} NID approved + Monetization activated!`)
  }

  const reject = async (u) => {
    const reason = prompt('Rejection reason:') || ''
    await supabase.from('profiles').update({ nid_status: 'rejected' }).eq('id', u.id)
    await sendSystemNotif(u.id, `❌ তোমার NID যাচাই হয়নি। কারণ: ${reason || 'Document unclear'}. আবার submit করো।`)
    setNidList(p => p.filter(x => x.id !== u.id))
  }

  if (loading) return <div style={{ textAlign:'center', padding:40, color:'#4a5568' }}>Loading...</div>
  if (!nidList.length) return (
    <div style={{ textAlign:'center', padding:40 }}>
      <div style={{ fontSize:48, marginBottom:8 }}>✅</div>
      <div style={{ color:'#4a5568', fontSize:13 }}>No pending NID requests</div>
    </div>
  )

  return nidList.map(u => (
    <div key={u.id} style={{ background:'#111826', border:'1px solid rgba(0,229,255,0.15)', borderRadius:14, padding:14, marginBottom:10 }}>
      <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:12 }}>
        <div style={{ width:40, height:40, borderRadius:'50%', background:'linear-gradient(135deg,#00e5ff,#00ff88)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:16, fontWeight:900, color:'#070a12' }}>
          {u.avatar_url ? <img src={u.avatar_url} style={{ width:'100%', height:'100%', objectFit:'cover', borderRadius:'50%' }}/> : (u.full_name||u.username||'U')[0].toUpperCase()}
        </div>
        <div>
          <div style={{ fontSize:13, fontWeight:700 }}>{u.full_name || u.username}</div>
          <div style={{ fontSize:10, color:'#4a5568' }}>@{u.username}</div>
        </div>
      </div>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:12 }}>
        {u.nid_front_url && (
          <div>
            <div style={{ fontSize:10, color:'#4a5568', marginBottom:4 }}>Front</div>
            <img src={u.nid_front_url} onClick={()=>window.open(u.nid_front_url,'_blank')} style={{ width:'100%', height:100, objectFit:'cover', borderRadius:8, cursor:'pointer', border:'1px solid rgba(255,255,255,0.1)' }}/>
          </div>
        )}
        {u.nid_back_url && (
          <div>
            <div style={{ fontSize:10, color:'#4a5568', marginBottom:4 }}>Back</div>
            <img src={u.nid_back_url} onClick={()=>window.open(u.nid_back_url,'_blank')} style={{ width:'100%', height:100, objectFit:'cover', borderRadius:8, cursor:'pointer', border:'1px solid rgba(255,255,255,0.1)' }}/>
          </div>
        )}
      </div>
      <div style={{ display:'flex', gap:8 }}>
        <button onClick={()=>approve(u)} style={{ flex:1, padding:'10px', background:'rgba(0,255,136,0.12)', border:'1px solid rgba(0,255,136,0.3)', borderRadius:10, color:'#00ff88', fontSize:13, fontWeight:700, cursor:'pointer' }}>
          ✅ Approve
        </button>
        <button onClick={()=>reject(u)} style={{ flex:1, padding:'10px', background:'rgba(255,69,96,0.12)', border:'1px solid rgba(255,69,96,0.3)', borderRadius:10, color:'#ff4560', fontSize:13, fontWeight:700, cursor:'pointer' }}>
          ❌ Reject
        </button>
      </div>
    </div>
...
export default function AdminPage() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data }) => {
      ...
      setUser(u)
      if (!ADMIN_EMAILS.includes(u.email)) { setLoading(false); return }
      setAuth(true)
      await refreshStats()
      await loadCommRates()
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#070a12', color:'#eef2f7', fontFamily:'system-ui,sans-serif' }}>
      ...
      <div style={{ padding:'115px 14px 40px' }}>
        ...
        {tab === 'nid' && (
          <div style={{ animation:'fadeUp .3s ease' }}>
            <div style={{ fontSize:'15px', fontWeight:'900', color:'#00e5ff', marginBottom:'12px' }}>🪪 NID Verification Requests</div>
            <NIDPanel supabase={supabase} sendSystemNotif={sendSystemNotif} />
          </div>
        )}
