...
export default function DM() {
  ...
  const startConversation = async (otherUser) => {
    ...
    if (existing) {
      const other = existing.user1_id === user.id ? existing.user2 : existing.user1
      ...
      <div style={{ height: '100vh', background: '#070a10', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ background: 'rgba(7,10,16,0.97)', borderBottom: '1px solid rgba(255,255,255,0.07)', padding: '12px 16px', display: 'flex', alignItems: 'center', gap: '12px', flexShrink: 0 }}>
          <button onClick={() => { setActiveConv(null); realtimeRef.current?.unsubscribe() }}
            style={{ background: 'none', border: 'none', color: '#8892a4', fontSize: '22px', cursor: 'pointer', flexShrink: 0 }}>←</button>
          <Avatar profile={other} size={40} />
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: '14px', fontWeight: '700', color: '#eef2f7', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{other?.full_name || other?.username}</div>
            <div style={{ fontSize: '11px', color: '#00e5ff' }}>@{other?.username}</div>
          </div>
          <button onClick={() => window.location.href = `/user/${other?.id}`}
            style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '10px', padding: '6px 12px', color: '#8892a4', fontSize: '12px', cursor: 'pointer' }}>
            Profile
          </button>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 12px' }}>
          {messages.length === 0 && (
            <div style={{ textAlign: 'center', padding: '60px 20px' }}>
              ...
              <div style={{ color: '#4a5568', fontSize: '14px' }}>Say hello to {other?.full_name || other?.username}!</div>
            </div>
          )}
          {messages.map((msg, i) => {
            ...
            return (
              <div key={msg.id} style={{ animation: 'fadeIn 0.2s ease' }}>
                {showTime && (
                  <div style={{ textAlign: 'center', fontSize: '10px', color: '#4a5568', margin: '8px 0' }}>
                ...
                <div style={{ display: 'flex', justifyContent: isMine ? 'flex-end' : 'flex-start', marginBottom: '6px' }}>
                  <div style={{
                    ...
                    background: isMine ? 'linear-gradient(135deg,#00e5ff,#00ff88)' : 'rgba(255,255,255,0.07)',
                    color: isMine ? '#070a10' : '#eef2f7',
                    fontSize: '14px',
                    lineHeight: '1.5',
                    fontWeight: isMine ? '500' : '400',
                    wordBreak: 'break-word',
                    boxShadow: isMine ? '0 2px 12px rgba(0,229,255,0.3)' : 'none',
                  }}>
                    {msg.content}
                  </div>
                </div>
        ...
        <div style={{ background: 'rgba(7,10,16,0.98)', borderTop: '1px solid rgba(255,255,255,0.07)', padding: '12px', display: 'flex', gap: '8px', alignItems: 'center', flexShrink: 0 }}>
          ...
          <button onClick={sendMessage} disabled={!text.trim() || sending}
            style={{ width: '44px', height: '44px', borderRadius: '50%', background: text.trim() ? 'linear-gradient(135deg,#00e5ff,#00ff88)' : 'rgba(255,255,255,0.05)', border: 'none', fontSize: '20px', cursor: text.trim() ? 'pointer' : 'default', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 0.2s' }}>
            {sending ? '⏳' : '↗'}
          </button>
        </div>
      </div>
    ...
    <div style={{ minHeight: '100vh', background: '#070a10', color: '#eef2f7' }}>
      ...
      <div style={{ position: 'fixed', top: 0, left: 0, right: 0, background: 'rgba(7,10,16,0.97)', borderBottom: '1px solid rgba(255,255,255,0.07)', zIndex: 100, padding: '12px 16px' }}>
        ...
        {searchResults.length > 0 && (
          <div style={{ background: '#111620', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '14px', marginTop: '8px', overflow: 'hidden' }}>
            {searchResults.map(u => (
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: 'rgba(7,10,16,0.98)', borderTop: '1px solid rgba(255,255,255,0.07)', display: 'flex', justifyContent: 'space-around', padding: '10px 0 20px', zIndex: 100 }}>
        {[
          ...
          { icon: '🗺', path: '/map' },
          { icon: '📸', path: '/post' },
          { icon: '🏆', path: '/leaderboard' },
          { icon: '👤', path: '/profile' }
        ].map(item => (
          <div key={item.path} onClick={() => window.location.href = item.path}
            style={{ fontSize: '22px', cursor: 'pointer', color: '#4a5568' }}>
            {item.icon}
          </div>
        ))}
      </div>

      <div style={{ padding: '140px 0 90px' }}>
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '60px 20px' }}>
            ...
            <div style={{ fontSize: '14px', color: '#4a5568' }}>Search for someone to start chatting!</div>
          </div>
        ) : conversations.map(conv => {
          const other = getOther(conv)
          return (
            <div key={conv.id} onClick={() => setActiveConv({ ...conv, other })}
              style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 16px', borderBottom: '1px solid rgba(255,255,255,0.04)', cursor: 'pointer', transition: 'background 0.2s' }}>
