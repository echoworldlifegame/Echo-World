...
export default function Home() {
  ...
  const handleSignup = async () => {
    ...
    if (signupStep === 3) {
      // বাধ্যতামূলক validation
      if (!phone.trim()) { setError('Phone number দিতে হবে'); return }
      if (!/^\+?[0-9]{10,15}$/.test(phone.replace(/\s/g,''))) { setError('সঠিক phone number দাও'); return }
      if (!dob) { setError('Date of Birth দিতে হবে'); return }
      if (!gender) { setError('Gender select করতে হবে'); return }
      setLoading(true); setError('')
      ...
      <div style={{ width:'100%', maxWidth:'400px', position:'relative', zIndex:1 }}>
            ...
            {authMode==='signup' && signupStep===3 && (
              <div>
                <div style={{ textAlign:'center', marginBottom:20 }}>
                  ...
                  <div style={{ fontSize:12, color:'#ff4560' }}>* সব তথ্য দেওয়া বাধ্যতামূলক</div>
                </div>
                <div style={{ marginBottom:12 }}>
                  <div style={{ fontSize:11, color:'#4a5568', fontWeight:600, marginBottom:6, letterSpacing:1 }}>PHONE NUMBER *</div>
                  <input value={phone} onChange={e=>setPhone(e.target.value)} type="tel" placeholder="01XXXXXXXXX"
                    style={{ width:'100%', background:'#0c1018', border:'1px solid rgba(255,255,255,.08)', borderRadius:12, padding:'12px 14px', color:'#eef2f7', fontSize:14, outline:'none' }}/>
                </div>
                <div style={{ marginBottom:12 }}>
                  <div style={{ fontSize:11, color:'#4a5568', fontWeight:600, marginBottom:6, letterSpacing:1 }}>DATE OF BIRTH *</div>
                ...
                <div style={{ marginBottom:12 }}>
                  <div style={{ fontSize:11, color:'#4a5568', fontWeight:600, marginBottom:8, letterSpacing:1 }}>GENDER *</div>
        ...
        {signupStep < 4 && (
          <div style={{ textAlign:'center', marginTop:20, fontSize:11, color:'#2a3040' }}>
            ...
            <span onClick={()=>window.location.href='/terms'} style={{ color:'#00e5ff', cursor:'pointer', textDecoration:'underline' }}>Terms & Privacy Policy</span>
          </div>
        )}
      </div>
    </div>
