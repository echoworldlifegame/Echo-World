...
export default function Home() {
  ...
  const verifyReferralCode = async (code) => {
    ...
    const { data } = await supabase
      ...
      .single()
    if (data) { setReferralValid(true); setReferralUsername(data.username) }
  ...
  const handleSignup = async () => {
    ...
    if (signupStep === 3) {
      ...
      if (data.user) {
        ...
        if (referrerId) sessionStorage.setItem('echo_referrer_id', referrerId)
  ...
  const handleOtpInput = (i, val) => {
    if (val.length > 1) {
      ...
      const arr = Array.from({ length: 6 }, (_, idx) => digits[idx] || '')
      setOtp(arr)
      setTimeout(() => inputRefs.current[Math.min(digits.length, 5)]?.focus(), 10)
      return
    }
    if (val && !/^\d$/.test(val)) return
    const arr = [...otp]; arr[i] = val; setOtp(arr)
    if (val && i < 5) setTimeout(() => inputRefs.current[i + 1]?.focus(), 10)
  }
  ...
  const verifyOTP = async (code) => {
    if (!email || code.length !== 6 || otpLoading) return
          ...
          <div style={{ animation:'fadeUp .35s ease' }}>
            ...
            <div style={{ textAlign:'center', marginBottom:28 }}>
              ...
              <div style={{ fontSize:13, color:'#8892a4', lineHeight:1.7 }}>
                ...
                একটি <span style={{ color:'#eef2f7', fontWeight:700 }}>6-সংখ্যার code</span> পাঠানো হয়েছে
              </div>
            </div>
