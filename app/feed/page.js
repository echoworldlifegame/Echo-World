...
📅 Posted: ${timeAgo(post.created_at)}`
      ...
      {showGift&&<div style={{position:'fixed',inset:0,zIndex:110,background:'rgba(0,0,0,.7)'}} onClick={()=>setShowGift(false)}>
        <div style={{position:'absolute',bottom:0,left:0,right:0,background:'#1a1f2e',borderRadius:'24px 24px 0 0',padding:'20px 16px 44px',animation:'slideUp .25s ease'}} onClick={e=>e.stopPropagation()}>
          <div style={{width:'40px',height:'4px',borderRadius:'2px',background:'rgba(255,255,255,.15)',margin:'0 auto 16px'}}/>
          <div style={{fontSize:'16px',fontWeight:'900',color:'#eef2f7',marginBottom:'4px',textAlign:'center'}}>🎁 Gift পাঠাও</div>
          <div style={{fontSize:'12px',color:'#4a5568',marginBottom:'20px',textAlign:'center'}}>to {getName(post)}</div>
          {giftSent?<div style={{textAlign:'center',padding:'20px'}}>
            ...
            {[{e:'🌹',n:'Rose',p:'5 Echo'},{e:'🎂',n:'Cake',p:'10 Echo'},{e:'💎',n:'Diamond',p:'50 Echo'},{e:'👑',n:'Crown',p:'100 Echo'},{e:'🚀',n:'Rocket',p:'200 Echo'},{e:'🏆',n:'Trophy',p:'500 Echo'},{e:'🌟',n:'Star',p:'20 Echo'},{e:'❤️',n:'Heart',p:'15 Echo'}].map((g,i)=>(
              <div key={i} onClick={()=>{setGiftSent(true);setTimeout(()=>{setGiftSent(false);setShowGift(false)},2000)}} style={{display:'flex',flexDirection:'column',alignItems:'center',gap:'4px',padding:'10px 4px',borderRadius:'12px',background:'rgba(255,255,255,.05)',border:'1px solid rgba(255,255,255,.08)',cursor:'pointer'}}>
                <span style={{fontSize:'26px'}}>{g.e}</span>
                <span style={{fontSize:'10px',color:'#eef2f7',fontWeight:'700'}}>{g.n}</span>
                <span style={{fontSize:'9px',color:'#4a5568'}}>{g.p}</span>
              </div>
            ))}
          </div>}
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.85)',zIndex:400,display:'flex',alignItems:'flex-end'}} onClick={()=>setShowLiveModal(false)}>
          <div style={{width:'100%',background:'linear-gradient(180deg,#0d1220 0%,#111827 100%)',borderRadius:'28px 28px 0 0',padding:'0 0 44px',animation:'slideUp .35s cubic-bezier(.25,.46,.45,.94)',maxHeight:'90vh',overflowY:'auto',border:'1px solid rgba(255,255,255,.08)',borderBottom:'none'}} onClick={e=>e.stopPropagation()}>
            ...
            {liveStep==='active'&&<>
              ...
              <div style={{padding:'0 16px'}}>
                <button onClick={()=>{ setShowLiveModal(false); window.location.href='/live/start' }} style={{width:'100%',padding:'16px',background:'linear-gradient(135deg,#ff4560,#ff6b35)',border:'none',borderRadius:'16px',color:'#fff',fontSize:'16px',fontWeight:'900',cursor:'pointer',marginBottom:'10px',boxShadow:'0 4px 24px rgba(255,69,96,.4)'}}>
                  🔴 Live শুরু করুন
                </button>
                <button onClick={()=>setShowLiveModal(false)} style={{width:'100%',padding:'14px',background:'rgba(255,255,255,.06)',border:'none',borderRadius:'14px',color:'#eef2f7',fontSize:'14px',fontWeight:'700',cursor:'pointer'}}>Cancel</button>
              </div>
            ...
            {liveStep==='free'&&<>
              <div style={{padding:'24px 20px',textAlign:'center'}}>
                ...
                <button onClick={()=>{ setShowLiveModal(false); window.location.href='/live/start' }} style={{width:'100%',padding:'16px',background:'linear-gradient(135deg,#ffd700,#ff6b35)',border:'none',borderRadius:'16px',color:'#070a12',fontSize:'16px',fontWeight:'900',cursor:'pointer',marginBottom:'10px',boxShadow:'0 4px 24px rgba(255,215,0,.35)'}}>
                  👑 Live শুরু করুন — FREE
                </button>
                <button onClick={()=>setShowLiveModal(false)} style={{width:'100%',padding:'14px',background:'rgba(255,255,255,.06)',border:'none',borderRadius:'14px',color:'#eef2f7',fontSize:'14px',fontWeight:'700',cursor:'pointer'}}>Cancel</button>
              </div>
