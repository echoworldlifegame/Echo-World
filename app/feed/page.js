...
export default function Feed() {
  ...
  const toggleAutoplay = () => {
    ...
    if (!next) Object.values(videoRefs.current).forEach(v=>v?.pause())
  }
        ...
        {loading ? (
          ...
          <div key={post.id} style={{background:'#111620',border:'1px solid rgba(255,255,255,0.07)',borderRadius:'16px',margin:'0 12px 14px',overflow:'hidden'}}>
            ...
            <div style={{display:'flex',alignItems:'center',gap:'10px',padding:'12px 14px 8px'}}>
              ...
              <div style={{fontSize:'10px',color:'#2a3040',flexShrink:0}}>{post.privacy==='private'?'🔒':post.privacy==='friends'?'👥':'🌍'}</div>
            </div>
            ...
            {post.media_url&&post.media_type==='video'&&(
              <video
                ...
                src={post.media_url} controls playsInline muted loop
