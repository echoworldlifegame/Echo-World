...
export default function Feed() {
  ...
  const trackInteraction = useCallback((postId, type, durationMs = 0) => {
    fetch('/api/algorithm/interact', {
      method: 'POST',
        ...
        {loading ? (
          ...
          <div key={post.id} data-post-id={post.id}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '12px 14px 8px', position: 'relative' }}>
              <div onClick={(e) => { e.stopPropagation(); post.profiles?.id && (window.location.href = `/user/${post.profiles.id}`) }}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  {user?.id !== post.user_id && (
                    <button onClick={(e) => { e.stopPropagation(); handleSupport(post.profiles?.id) }}
            ...
            <div style={{ display: 'flex', padding: '4px 6px 10px', borderTop: '1px solid rgba(255,255,255,0.04)', gap: '4px' }}>
              ...
              <button onClick={(e) => { e.stopPropagation(); trackInteraction(post.id, 'comment'); window.location.href = `/comments/${post.id}` }}
              ...
              <button onClick={(e) => { e.stopPropagation(); trackInteraction(post.id, 'share'); navigator.share?.({ text: post.content || '', url: `${window.location.origin}/comments/${post.id}` }) }}
                style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '8px 12px', border: 'none', background: 'none', cursor: 'pointer', color: '#4a5568', fontSize: '13px', borderRadius: '8px' }}>
                ↗ Share
              </button>
              {post.media_type === 'video' && (
                <button onClick={(e) => { e.stopPropagation(); trackInteraction(post.id, 'remix'); window.location.href = '/echo' }}
              ...
              <button onClick={(e) => { e.stopPropagation(); window.location.href = '/map' }}
