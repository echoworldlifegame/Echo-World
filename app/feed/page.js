...
export default function Feed() {
    ...
    supabase.auth.getSession().then(async ({ data }) => {
      ...
      if (convs?.length) {
        ...
        const { count: unread } = await supabase
          ...
          .in('conversation_id', convIds).eq('read', false).neq('sender_id', u.id)
        setDmCount(unread || 0)
      }
    })
  ...
  const loadData = async (userId) => {
    ...
    const creatorCount = {}
    const sorted = scored.filter(p => {
      creatorCount[p.user_id] = (creatorCount[p.user_id] || 0) + 1
      return creatorCount[p.user_id] <= 2
    })
    setPosts(sorted)
    ...
    const { data: st } = await supabase.from('stories')
      .select('*, profiles(id, username, full_name, avatar_url)')
      .gt('expires_at', storyNow).order('created_at', { ascending: false })
    const groups = {}
    ;(st || []).forEach(s => {
  ...
  const handleInstall = async () => {
    if (deferredPrompt) {
      try { deferredPrompt.prompt(); const { outcome } = await deferredPrompt.userChoice; if (outcome === 'accepted') setInstalled(true); setDeferredPrompt(null) } catch (e) { console.log(e) }
      return
    }
    const isIOS = /iphone|ipad|ipod/i.test(navigator.userAgent)
    if (isIOS) { alert('iOS তে Install করতে:\n\n১. নিচে Share বাটনে চাপো\n২. "Add to Home Screen" সিলেক্ট করো\n৩. "Add" চাপো'); return }
  ...
  const handleLike = async (post) => {
    ...
    if (liked[post.id]) {
      ...
      if (post.user_id !== user.id) {
        await supabase.from('notifications').insert({ user_id: post.user_id, from_user_id: user.id, type: 'like', post_id: post.id })
      }
    }
  }
  ...
  const handleSupport = async (profileId) => {
    ...
    if (supported[profileId]) {
      ...
      await supabase.from('notifications').insert({ user_id: profileId, from_user_id: user.id, type: 'follow' })
    }
  }
      ...
      <div style={{ position: 'fixed', top: 0, left: 0, right: 0, background: 'rgba(7,10,16,0.97)', borderBottom: '1px solid rgba(255,255,255,0.07)', zIndex: 100 }}>
        <div style={{ padding: '0 12px', height: '54px', display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <div style={{ position: 'relative', flexShrink: 0 }}>
            <button onClick={() => window.location.href = '/dm'}
              style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '50%', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: '16px' }}>✉️</button>
          ...
          <div style={{ position: 'relative', flexShrink: 0 }}>
            <button onClick={() => window.location.href = '/notifications'}
              style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '50%', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: '16px' }}>🔔</button>
          ...
          <button onClick={() => window.location.href = '/settings'}
            style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '50%', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: '16px', flexShrink: 0 }}>⚙️</button>
        </div>

        {/* TABS — ECHO এর পাশে Community */}
        <div style={{ display: 'flex', gap: '6px', padding: '0 12px 10px', overflowX: 'auto', scrollbarWidth: 'none' }}>
          {[
            { key: 'all', label: '🌍 All' },
            { key: 'echo', label: '⚡ ECHO', direct: '/echo' },
            { key: 'community', label: '👥 Community', direct: '/community' },
            { key: 'photos', label: '📷 Photos' },
            { key: 'capsules', label: '📦 Capsules' },
          ].map(t => (
            <button key={t.key} onClick={() => t.direct ? window.location.href = t.direct : setActiveTab(t.key)}
              style={{ padding: '6px 14px', borderRadius: '20px', border: 'none', cursor: 'pointer', fontSize: '12px', fontWeight: '700', whiteSpace: 'nowrap', flexShrink: 0, background: activeTab === t.key && !t.direct ? 'linear-gradient(135deg,#00e5ff,#00ff88)' : t.key === 'community' ? 'rgba(0,229,255,0.08)' : 'rgba(255,255,255,0.07)', color: activeTab === t.key && !t.direct ? '#070a10' : t.key === 'community' ? '#00e5ff' : '#8892a4', border: t.key === 'community' ? '1px solid rgba(0,229,255,0.2)' : 'none' }}>
              {t.label}
            </button>
          ))}
        </div>
            ...
            <div key={i} onClick={() => window.location.href = '/stories'}
              ...
              <div style={{ width: '62px', height: '62px', borderRadius: '50%', padding: '2px', background: 'linear-gradient(135deg,#00e5ff,#00ff88,#ffa500)', boxShadow: '0 0 14px rgba(0,229,255,0.35)' }}>
                <div style={{ width: '100%', height: '100%', borderRadius: '50%', overflow: 'hidden', background: '#111620', border: '2px solid #070a10', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  {s.user?.avatar_url ? <img src={s.user.avatar_url} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ fontSize: '22px', fontWeight: '900', color: '#00e5ff' }}>{(s.user?.full_name || s.user?.username || 'E')[0].toUpperCase()}</span>}
                </div>
              </div>
              <div style={{ fontSize: '9px', color: '#8892a4', fontWeight: '600', maxWidth: '62px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', textAlign: 'center' }}>{s.user?.username}</div>
            </div>
          ))}
        </div>
          ...
          <div key={post.id} style={{ background: '#111620', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '16px', margin: '0 12px 14px', overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '12px 14px 8px' }}>
              <div onClick={() => post.profiles?.id && (window.location.href = `/user/${post.profiles.id}`)}
                ...
                {post.profiles?.avatar_url ? <img src={post.profiles.avatar_url} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <span style={{ fontSize: '16px', fontWeight: '800', color: '#070a10' }}>{getName(post)[0]?.toUpperCase()}</span>}
              </div>
            ...
            {post.media_url && post.media_type === 'photo' && <img src={post.media_url} style={{ width: '100%', maxHeight: '450px', objectFit: 'cover', display: 'block' }} />}
                  ...
                  {(!dur || isShort) && (
                    <div style={{ position:'relative' }}>
                      <video ref={el=>{if(el){videoRefs.current[post.id]=el;observerRef.current?.observe(el)}}}
                        src={post.media_url} playsInline muted loop style={{width:'100%',maxHeight:'450px',display:'block',background:'#000',cursor:'pointer'}}
                        onClick={()=>window.location.href=`/video?id=${post.id}`} />
                      <div style={{position:'absolute',top:'10px',left:'10px',background:'rgba(255,0,0,.9)',borderRadius:'6px',padding:'2px 8px',fontSize:'10px',fontWeight:'800',color:'#fff',pointerEvents:'none'}}>SHORT</div>
                      {dur>0&&<div style={{position:'absolute',bottom:'10px',right:'10px',background:'rgba(0,0,0,.75)',borderRadius:'4px',padding:'2px 7px',fontSize:'11px',fontWeight:'700',color:'#fff',pointerEvents:'none'}}>{fmtDur(dur)}</div>}
                      <div onClick={()=>window.location.href=`/video?id=${post.id}`} style={{position:'absolute',top:'10px',right:'10px',background:'rgba(0,0,0,.6)',borderRadius:'50%',width:'32px',height:'32px',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',fontSize:'14px'}}>{'⛶'}</div>
                    </div>
                  )}
                  {isLong && (
                    <div onClick={()=>window.location.href=`/video?id=${post.id}`} style={{position:'relative',background:'#000',cursor:'pointer',paddingBottom:'56.25%',overflow:'hidden'}}>
                      {post.image_url
                      ...
                      <div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(0,0,0,.25)'}}>
                        <div style={{width:'56px',height:'56px',borderRadius:'50%',background:'rgba(0,0,0,.75)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'24px',border:'3px solid rgba(255,255,255,.85)'}}>{'▶'}</div>
                      </div>
            ...
            <div style={{ display: 'flex', padding: '4px 6px 10px', borderTop: '1px solid rgba(255,255,255,0.04)', gap: '4px' }}>
              ...
              <button onClick={() => navigator.share?.({ text: post.content || '', url: `${window.location.origin}/comments/${post.id}` })}
                style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '8px 12px', border: 'none', background: 'none', cursor: 'pointer', color: '#4a5568', fontSize: '13px', borderRadius: '8px' }}>
                ↗ Share
              </button>
