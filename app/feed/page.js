    ...
    <div style={{position:'fixed',inset:0,background:'#0a0d14',zIndex:500,display:'flex',flexDirection:'column',overflowY:'auto'}}>
      ...
      <div ref={containerRef} style={{position:'sticky',top:0,zIndex:20,background:'#000',userSelect:'none'}}
        ...
        <video ref={videoRef} src={post.media_url} poster={post.image_url}
          style={{width:'100%',display:'block',maxHeight:fullscreen?'100vh':'250px',objectFit:'contain',background:'#000'}}
          onTimeUpdate={e=>{
...
function ShortViewer({ posts, startIdx, onClose, user, liked, onLike, supported, onSupport }) {
  ...
  const goNext = () => { if(idx<posts.length-1) setIdx(p=>p+1) }
  ...
  const handleDoubleTap = () => {
    if(!user||!post) return
    if(!localLiked[post.id]){
      supabase.from('likes').insert({post_id:post.id,user_id:user.id})
      setLocalLiked(p=>({...p,[post.id]:true}))
      setLocalLikes(p=>({...p,[post.id]:(p[post.id]||0)+1}))
      onLike(post)
    }
  ...
  const toggleLikeBtn = () => {
    ...
    if(localLiked[post.id]){
      supabase.from('likes').delete().eq('post_id',post.id).eq('user_id',user.id)
      setLocalLiked(p=>({...p,[post.id]:false}))
      setLocalLikes(p=>({...p,[post.id]:Math.max(0,(p[post.id]||1)-1)}))
    } else {
      supabase.from('likes').insert({post_id:post.id,user_id:user.id})
      setLocalLiked(p=>({...p,[post.id]:true}))
      setLocalLikes(p=>({...p,[post.id]:(p[post.id]||0)+1}))
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'#000',zIndex:500}}
      ...
      <style>{`@keyframes heartPop{0%{transform:translate(-50%,-50%) scale(0)}50%{transform:translate(-50%,-50%) scale(1.4)}100%{transform:translate(-50%,-50%) scale(1) opacity(0)}}`}</style>
      ...
      {/* Top bar */}
      <div style={{position:'absolute',top:0,left:0,right:0,padding:'16px',display:'flex',alignItems:'center',gap:'12px',background:'linear-gradient(rgba(0,0,0,.5),transparent)',zIndex:10}}>
      ...
      {/* Pause indicator */}
      {!playing&&(
        <div style={{position:'absolute',top:'50%',left:'50%',transform:'translate(-50%,-50%)',fontSize:'60px',opacity:.8,pointerEvents:'none'}}>⏸</div>
      )}

      {/* Double tap heart */}
      {showHeart&&(
        <div style={{position:'absolute',top:'40%',left:'50%',transform:'translate(-50%,-50%)',fontSize:'80px',animation:'heartPop .8s ease',pointerEvents:'none',zIndex:20}}>❤️</div>
      )}

      {/* Right actions */}
      <div style={{position:'absolute',right:'14px',bottom:'130px',display:'flex',flexDirection:'column',gap:'18px',alignItems:'center',zIndex:10}}>
        {/* Avatar */}
        <div style={{position:'relative'}}>
          <div onClick={()=>window.location.href=`/user/${post.profiles?.id}`}
            style={{width:'46px',height:'46px',borderRadius:'50%',overflow:'hidden',border:'2px solid #fff',cursor:'pointer',background:'linear-gradient(135deg,#00e5ff,#00ff88)',display:'flex',alignItems:'center',justifyContent:'center'}}>
            {getAvatar(post.profiles,46)}
          </div>
          {user?.id!==post.user_id&&!supported[post.profiles?.id]&&(
            <div onClick={()=>onSupport(post.profiles?.id)}
              style={{position:'absolute',bottom:'-8px',left:'50%',transform:'translateX(-50%)',width:'18px',height:'18px',borderRadius:'50%',background:'#ff4560',display:'flex',alignItems:'center',justifyContent:'center',cursor:'pointer',fontSize:'14px',fontWeight:'800',color:'#fff'}}>+</div>
          )}
        </div>
        {/* Like */}
        <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:'3px'}}>
          <button onClick={toggleLikeBtn} style={{background:'none',border:'none',cursor:'pointer',fontSize:'32px',filter:localLiked[post.id]?'drop-shadow(0 0 8px #ff4560)':'none'}}>{localLiked[post.id]?'❤️':'🤍'}</button>
          <span style={{color:'#fff',fontSize:'11px',fontWeight:'700',textShadow:'0 1px 3px rgba(0,0,0,.8)'}}>{fmtCount(localLikes[post.id]||0)}</span>
        </div>
        {/* Comment */}
        <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:'3px'}}>
          <button style={{background:'none',border:'none',cursor:'pointer',fontSize:'30px'}}>💬</button>
          <span style={{color:'#fff',fontSize:'11px',fontWeight:'700',textShadow:'0 1px 3px rgba(0,0,0,.8)'}}>{fmtCount(post.comments_count||0)}</span>
        </div>
        {/* Share */}
        <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:'3px'}}>
          <button onClick={()=>navigator.share?.({url:window.location.origin})||navigator.clipboard?.writeText(window.location.origin)}
            style={{background:'none',border:'none',cursor:'pointer',fontSize:'28px'}}>⬆️</button>
          <span style={{color:'#fff',fontSize:'11px',fontWeight:'700',textShadow:'0 1px 3px rgba(0,0,0,.8)'}}>Share</span>
        </div>
        {/* More */}
        <button style={{background:'none',border:'none',cursor:'pointer',fontSize:'26px'}}>⋯</button>
      </div>

      {/* Bottom info */}
      <div style={{position:'absolute',bottom:0,left:0,right:'70px',padding:'16px 16px 36px',background:'linear-gradient(transparent,rgba(0,0,0,.75))',zIndex:10}}>
        <div style={{display:'flex',alignItems:'center',gap:'8px',marginBottom:'8px'}}>
          <span style={{fontSize:'14px',fontWeight:'800',color:'#fff',cursor:'pointer'}} onClick={()=>window.location.href=`/user/${post.profiles?.id}`}>{getName(post)}</span>
          <span style={{fontSize:'11px',color:'rgba(255,255,255,.6)'}}>{timeAgo(post.created_at)}</span>
        </div>
        {post.content&&<div style={{fontSize:'13px',color:'rgba(255,255,255,.9)',lineHeight:'1.5',marginBottom:'6px',overflow:'hidden',display:'-webkit-box',WebkitLineClamp:2,WebkitBoxOrient:'vertical'}}>{post.content}</div>}
        {post.hashtags&&<div style={{fontSize:'12px',color:'#00e5ff',fontWeight:'600'}}>{post.hashtags}</div>}
        {/* Music / sound bar */}
        <div style={{display:'flex',alignItems:'center',gap:'8px',marginTop:'8px'}}>
          <span style={{fontSize:'14px',animation:'spin 3s linear infinite',display:'inline-block'}}>🎵</span>
          <span style={{fontSize:'11px',color:'rgba(255,255,255,.7)',flex:1,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>Echo World Original Sound</span>
        </div>
      </div>
      ...
      <div style={{position:'absolute',right:'6px',top:'50%',transform:'translateY(-50%)',display:'flex',flexDirection:'column',gap:'5px',zIndex:10}}>
        {posts.slice(Math.max(0,idx-3),Math.min(posts.length,idx+4)).map((_,i)=>{
          ...
          return <div key={realIdx} style={{width:'3px',height:realIdx===idx?'20px':'5px',borderRadius:'2px',background:'rgba(255,255,255,.7)',transition:'height .2s'}}/>
        })}
      </div>
    </div>
  )
