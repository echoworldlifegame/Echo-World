  ...
  useEffect(() => {
    ...
    if (user?.id) {
      supabase.from('profiles').select('coin_balance').eq('id', user.id).single()
        .then(({ data }) => { if (data) setUserCoinBal(data.coin_balance || 0) })
    }
  }, [user?.id])
            ...
            {[{e:'🌹',n:'Rose',c:5},{e:'🎂',n:'Cake',c:10},{e:'💎',n:'Diamond',c:50},{e:'👑',n:'Crown',c:100},{e:'🚀',n:'Rocket',c:200},{e:'🏆',n:'Trophy',c:500},{e:'🌟',n:'Star',c:20},{e:'❤️',n:'Heart',c:15}].map((g,i)=>{
              ...
              <div key={i} onClick={async()=>{
                ...
                try {
                  ...
                  await supabase.from('notifications').insert({user_id:post.user_id,type:'gift',read:false,message:`🎁 তুমি ${g.e} ${g.n} gift পেয়েছো! (+${recv} coins)`})
                  setGiftSent(true)
