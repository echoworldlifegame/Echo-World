...
function YouTubePlayer({ post, onClose, user, liked, onLike, supported, onSupport, onNext }) {
  ...
  useEffect(() => {
    ...
    try{
      ...
      setTimeout(()=>{ try{ videoRef.current?.play().then(()=>setPlaying(true)).catch(()=>{}) }catch(e){} },400)
    }catch(e){ console.error('YouTubePlayer init error:',e) }
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'#0a0d14',zIndex:500,display:'flex',flexDirection:'column',overflowY:'auto'}}>
      ...
      <div ref={containerRef} style={{position:'sticky',top:0,zIndex:20,background:'#000',userSelect:'none'}}
        ...
        <video ref={videoRef} src={post?.media_url||post?.video_url||''} poster={post?.image_url||''}
...
function ShortViewer({ posts, startIdx, onClose, user, liked, onLike, supported, onSupport }) {
  ...
  useEffect(() => {
    const v=videoRef.current; if(!v) return
    try{ v.currentTime=0; v.play().then(()=>setPlaying(true)).catch(()=>{}) }catch(e){}
    if(idx!==prevIdx){
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'#000',zIndex:500,overflow:'hidden'}}
      ...
      <div style={{position:'absolute',inset:0,
        ...
        <video ref={videoRef} src={post?.media_url||post?.video_url||''}
...
export default function Feed() {
  ...
  const loadData = async (userId) => {
    ...
    setPosts(sorted)
    setShortsList(sorted.filter(p=>p.media_type==='video'||p.media_url?.match(/\.(mp4|webm|mov|avi)/i)))
  ...
  const [shortViewerPost, setShortViewerPost] = useState(null)

  const openShortVideo = (post) => {
    if(!post?.id) return
    setShortViewerPost(post)
    if(shortsList.length===0){
      setOpenShortIdx(0)
      return
    }
    const foundIdx=shortsList.findIndex(p=>p.id===post.id)
    setOpenShortIdx(foundIdx>=0?foundIdx:0)
  }
  ...
  return (
    <div style={{minHeight:'100vh',background:'#070a10',color:'#eef2f7',fontFamily:'system-ui,sans-serif',paddingBottom:'90px'}}>
      ...
      {openShortIdx!==null&&<ShortViewer posts={shortsList.length>0?shortsList:(shortViewerPost?[shortViewerPost]:[])} startIdx={shortsList.length>0?openShortIdx:0} onClose={()=>{setOpenShortIdx(null);setShortViewerPost(null)}} user={user} liked={liked} onLike={handleLike} supported={supported} onSupport={handleSupport}/>}
