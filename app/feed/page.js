...
export default function Feed() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#070a10', color: '#eef2f7' }}>
        ...
        <div style={{ padding: '0 16px', height: '52px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            ...
            <div style={{ position: 'relative' }}>
              <button onClick={() => window.location.href = '/notifications'} style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '20px', padding: '6px 12px', color: '#8892a4', fontSize: '13px', cursor: 'pointer' }}>🔔</button>
            ...
            {showInstall ? (
              ...
              }} style={{ background: 'linear-gradient(135deg,#00e5ff,#00ff88)', border: 'none', borderRadius: '20px', padding: '6px 14px', color: '#070a10', fontSize: '12px', fontWeight: '700', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px' }}>
                📲 Install
              </button>
            ) : (
      ...
      {/* CONTENT */}
      <div style={{ padding: '108px 0 90px', maxWidth: '600px', margin: '0 auto' }}>

        {/* Create post bar */}
        <div onClick={() => window.location.href = '/post'} style={{ background: '#111620', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '16px', padding: '12px 14px', margin: '0 12px 14px', display: 'flex', gap: '10px', alignItems: 'center', cursor: 'pointer' }}>
        ...
        {loading ? (
          ...
          <div key={post.id} style={{ background: '#111620', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '16px', margin: '0 12px 14px', overflow: 'hidden' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  {user?.id !== post.user_id && (
                    <button onClick={() => handleSupport(post.profiles?.id)} style={{ padding: '2px 10px', borderRadius: '12px', border: `1px solid ${supported[post.profiles?.id] ? 'rgba(0,229,255,0.3)' : 'rgba(255,255,255,0.15)'}`, background: supported[post.profiles?.id] ? 'rgba(0,229,255,0.08)' : 'transparent', color: supported[post.profiles?.id] ? '#00e5ff' : '#4a5568', fontSize: '10px', fontWeight: '600', cursor: 'pointer' }}>
            ...
            <div style={{ display: 'flex', padding: '4px 6px 10px', borderTop: '1px solid rgba(255,255,255,0.04)', gap: '4px' }}>
              <button onClick={() => handleLike(post)} style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '8px 12px', border: 'none', background: 'none', cursor: 'pointer', color: liked[post.id] ? '#ff4560' : '#4a5568', fontSize: '13px', borderRadius: '8px', transition: 'all 0.2s', transform: liked[post.id] ? 'scale(1.1)' : 'scale(1)' }}>
              ...
              <button onClick={() => navigator.share?.({ text: post.content || '', url: `${window.location.origin}/post/${post.id}` })} style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '8px 12px', border: 'none', background: 'none', cursor: 'pointer', color: '#4a5568', fontSize: '13px', borderRadius: '8px' }}>
                ↗ Share
              </button>
