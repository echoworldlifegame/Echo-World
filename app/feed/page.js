          ...
          {giftSent?<div style={{textAlign:'center',padding:'20px'}}>
            ...
            {[{e:'🌹',n:'Rose',c:5},{e:'🎂',n:'Cake',c:10},{e:'💎',n:'Diamond',c:50},{e:'👑',n:'Crown',c:100},{e:'🚀',n:'Rocket',c:200},{e:'🏆',n:'Trophy',c:500},{e:'🌟',n:'Star',c:20},{e:'❤️',n:'Heart',c:15}].map((g,i)=>{
              ...
              <div key={i} onClick={async()=>{
                if(!canAfford){alert(`🪙 Coin কম!\nদরকার: ${g.c}\nআছে: ${userCoinBal}\n\nProfile > Coin Store থেকে কিনো`);return}
                if(!user||!post?.user_id) return
                // Deduct from sender
                const newBal = userCoinBal - g.c
                await supabase.from('profiles').update({coin_balance:newBal}).eq('id',user.id)
                setUserCoinBal(newBal)
                // Add transaction for sender
                await supabase.from('coin_transactions').insert({user_id:user.id,amount:-g.c,type:'gift_sent',note:`${g.e} ${g.n} sent to @${getName(post)}`})
                // Add to receiver (80%)
                ...
                await supabase.from('coin_transactions').insert({user_id:post.user_id,amount:recv,type:'gift_received',note:`${g.e} ${g.n} received from viewer`})
                // Notify receiver
                await supabase.from('notifications').insert({user_id:post.user_id,type:'gift',message:`🎁 তুমি ${g.e} ${g.n} gift পেয়েছো! (+${recv} coins)`})
                setGiftSent(true);setTimeout(()=>{setGiftSent(false);setShowGift(false)},2000)
