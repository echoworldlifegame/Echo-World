...
export default function Feed() {
  ...
  const handleInstall = async () => {
    ...
    alert('Chrome ব্যবহার করুন এবং কিছুক্ষণ পর আবার চেষ্টা করুন।')
  }
      ...
      <div style={{ position: 'fixed', top: 0, left: 0, right: 0, background: 'rgba(7,10,16,0.97)', borderBottom: '1px solid rgba(255,255,255,0.07)', zIndex: 100 }}>
        <div style={{ padding: '0 12px', height: '54px', display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <button onClick={handleInstall} title={installed ? 'Installed ✓' : 'Install App'}
            ...
            <span style={{ fontSize: '7px', fontWeight: '900', color: installed ? '#00ff88' : '#070a10', lineHeight: 1, letterSpacing: '0.5px' }}>APP</span>
          </button>
      ...
      <div style={{ padding: '110px 0 90px', maxWidth: '600px', margin: '0 auto' }}>
        ...
        {loading ? (
          ...
          <div key={post.id} style={{ background: '#111620', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '16px', margin: '0 12px 14px', overflow: 'hidden' }}>
            ...
            {post.content && (
              <div style={{ padding: '8px 14px', fontSize: '14px', color: '#c0c8d8', lineHeight: '1.6' }}>{post.content}</div>
            )}
            {post.hashtags && (
              <div style={{ padding: '0 14px 8px', fontSize: '12px', color: '#00e5ff' }}>{post.hashtags}</div>
            )}
