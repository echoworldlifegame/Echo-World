...
export default function Echo() {
  ...
  const [transDir, setTransDir] = useState(0) // -1 up, 1 down
  ...
  }, [currentIndex, videos.length, transitioning])
  ...
  if (loading) return (
    <div style={{height:'100vh',background:'#000',display:'flex',alignItems:'center',justifyContent:'center',flexDirection:'column',gap:'12px'}}>
      ...
      <div style={{color:'#4a5568',fontSize:'14px',letterSpacing:'2px'}}>LOADING ECHO...</div>
  ...
  if (mixMode && mixVideo) {
    ...
    if (mixStep === 'camera') return (
      <div style={{minHeight:'100vh',background:'#000',color:'#fff',paddingBottom:'80px'}}>
        ...
        <div style={{paddingTop:'56px',display:'flex',flexDirection:'column',alignItems:'center',gap:'16px',padding:'72px 16px 20px'}}>
          ...
          <div style={{fontSize:'12px',color:'#4a5568',textAlign:'center'}}>Original video will be merged with your recording</div>
          {!cameraStream ? (
    ...
    if (mixStep === 'edit') {
      const renderLayout = () => {
        ...
        if (mixLayout==='side') return (
          <div style={{display:'flex',height:'240px'}}>
            <div style={{flex:1,overflow:'hidden'}}>{orig}</div>
            <div style={{flex:1,overflow:'hidden'}}>{mine}</div>
          </div>
        )
        if (mixLayout==='top') return (
          <div style={{height:'280px'}}>
            <div style={{height:'140px',overflow:'hidden'}}>{orig}</div>
            <div style={{height:'140px',overflow:'hidden'}}>{mine}</div>
          </div>
        )
        if (mixLayout==='pip-me') return (
          <div style={{height:'260px',position:'relative',overflow:'hidden'}}>
            <div style={{position:'absolute',inset:0}}>{mine}</div>
            <div style={{position:'absolute',top:'10px',right:'10px',width:'90px',height:'130px',borderRadius:'10px',overflow:'hidden',border:'2px solid #ffa500'}}>{orig}</div>
          </div>
        )
        if (mixLayout==='pip-them') return (
          <div style={{height:'260px',position:'relative',overflow:'hidden'}}>
            <div style={{position:'absolute',inset:0}}>{orig}</div>
            <div style={{position:'absolute',top:'10px',right:'10px',width:'90px',height:'130px',borderRadius:'10px',overflow:'hidden',border:'2px solid #ffa500'}}>{mine}</div>
          </div>
        )
        if (mixLayout==='split-v') return (
          <div style={{display:'flex',height:'240px'}}>
            <div style={{width:'50%',overflow:'hidden',clipPath:'polygon(0 0,85% 0,100% 100%,0 100%)'}}>{orig}</div>
            <div style={{width:'50%',overflow:'hidden',clipPath:'polygon(15% 0,100% 0,100% 100%,0 100%)'}}>{mine}</div>
          </div>
        )
        return (
          <div style={{height:'260px',position:'relative',overflow:'hidden'}}>
            <div style={{position:'absolute',inset:0}}>{orig}</div>
            <div style={{position:'absolute',inset:0,opacity:0.6}}>{mine}</div>
          </div>
        )
      }
            ...
            {activeEditTab==='text'&&(
              <div>
                ...
                <div style={{display:'flex',justifyContent:'space-between',marginBottom:'4px'}}>
                  ...
                  <span style={{fontSize:'12px',color:'#ffa500'}}>{textSize}px</span>
                </div>
      ...
      <div style={{minHeight:'100vh',background:'#070a10',color:'#eef2f7',paddingBottom:'80px'}}>
        ...
        <div style={{padding:'72px 16px 20px'}}>
          <div style={{borderRadius:'16px',overflow:'hidden',marginBottom:'16px',background:'#000',position:'relative'}}>
            ...
            <div style={{position:'absolute',top:'8px',left:'8px',background:'rgba(0,0,0,0.7)',borderRadius:'8px',padding:'4px 10px',fontSize:'11px',fontWeight:'900',background:'linear-gradient(90deg,#00e5ff22,#00ff8822)',color:'#00e5ff',backdropFilter:'blur(4px)'}}>⬡ ECHO WORLD</div>
          </div>
    ...
    <div style={{height:'100vh',background:'#000',overflow:'hidden',position:'relative',userSelect:'none'}}
      ...
      <video ref={videoRef} key={vid.id} src={vid.media_url}
        style={{position:'absolute',top:0,left:0,width:'100%',height:'100%',objectFit:'cover',
          transform: transitioning
            ? `translateY(${transDir < 0 ? '100%' : '-100%'})`
            : 'translateY(0)',
          transition: transitioning ? 'transform 0.35s cubic-bezier(0.25,0.46,0.45,0.94)' : 'none'
        }}
        loop={false} playsInline autoPlay onEnded={handleVideoEnd}/>

      {/* Next video peek during transition */}
      ...
      {showOutro&&(
        <div style={{position:'absolute',inset:0,background:'rgba(7,10,16,0.96)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',zIndex:450,animation:'fadeIn 0.4s ease'}}>
          ...
          <div style={{fontSize:'38px',fontWeight:'900',background:'linear-gradient(90deg,#00e5ff,#00ff88)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',letterSpacing:'-1px',animation:'slideUp 0.5s ease'}}>ECHO WORLD</div>
          <div style={{fontSize:'13px',color:'#4a5568',marginTop:'8px',animation:'slideUp 0.7s ease'}}>echo-world-psi.vercel.app</div>
      ...
      <div style={{position:'absolute',bottom:'92px',left:'16px',right:'80px',zIndex:200}}>
        <div style={{display:'flex',alignItems:'center',gap:'10px',marginBottom:'8px'}}>
          ...
          <div onClick={(e)=>{e.stopPropagation();vid.profiles?.id&&(window.location.href=`/user/${vid.profiles.id}`)}} style={{cursor:'pointer'}}>
            <div style={{color:'#fff',fontWeight:'800',fontSize:'15px',textShadow:'0 1px 4px rgba(0,0,0,0.8)'}}>{getName(vid)}</div>
      ...
      <div style={{position:'absolute',bottom:'92px',right:'10px',zIndex:200,display:'flex',flexDirection:'column',gap:'16px',alignItems:'center'}}>
        ...
        {/* Comment */}
        <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:'2px'}}>
          <button onClick={(e)=>{e.stopPropagation();loadComments(vid.id);setShowComments(true)}} style={{width:'48px',height:'48px',borderRadius:'50%',background:'rgba(255,255,255,0.1)',border:'2px solid rgba(255,255,255,0.4)',fontSize:'22px',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',backdropFilter:'blur(8px)'}}>💬</button>
          <span style={{color:'#fff',fontSize:'11px',fontWeight:'700'}}>{formatNum(vid.comments_count)}</span>
        </div>

        {/* Share */}
        <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:'2px'}}>
          <button onClick={(e)=>{e.stopPropagation();navigator.share?.({text:vid.content||'',url:window.location.href})}} style={{width:'48px',height:'48px',borderRadius:'50%',background:'rgba(255,255,255,0.1)',border:'2px solid rgba(255,255,255,0.4)',fontSize:'20px',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',backdropFilter:'blur(8px)'}}>↗</button>
